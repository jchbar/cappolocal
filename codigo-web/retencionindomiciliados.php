...
$pdf->MultiCell(0,0,"Indomiciliados Concepto Retención UCLA para Caja de Ahorro del ".convertir_fechadmy($fechanomina),0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',8);
$pdf->SetFillColor(255,255,255);
$pdf->SetTextColor(0);
$linea+=5;
$header=array('Cédula','Nombre','Retención','Com.Bco','Cédula','Nombre','Retención','Com.Bco');
$comisionbancaria = 5;
...
$sql="select cedula, substr(concat(trim(ape_prof), ' ',trim(nombr_prof)),1,25) as nombre, hab_prof, pertenece from t_his200, sgcaf200 where (fecha ='$fechanomina') and (cedula = ced_prof) and (hab_prof > 0) order by cedula";
...
while ($row = mysql_fetch_array($result))
{
 ...
 //imprimo nombre, apellidos y localidad
	...
	$pdf->Cell($w[2+$columna],5,number_format($row['hab_prof'],2,".",","),0,0,'R',0);
