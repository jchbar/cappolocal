...
if (!$accion) {
	echo "<div id='div1'>";
	echo "<form action='frm25.php?accion=ListadoDeCuotas' name='form1' method='post'>";
...
//	$fechanueva=$fechanueva[0].'/'.$fechanueva[1].'/'.$fechanueva[2];
	...
	$sqlano="SELECT substr(f_1cuo_sdp,1,4) as ano, count(f_1cuo_sdp) as cuantos FROM sgcaf310 WHERE (codpre_sdp = '004') AND stapre_sdp = 'A' AND renovado =0 group by f_1cuo_sdp ORDER BY f_1cuo_sdp limit 1";
...
*/
	...
	$sqlprestamos="select * from sgcaf360 where (cod_pres='004') order by cod_pres";
	...
	echo '</select> *'; 
	echo '</td>';
	echo '<br>Numero de Semanas a Procesar <input type="number" name="nrosemanas" value = 1 align="right"/><br />';
...
if (($accion=='ListadoDeCuotas') and (!isset($_POST['descontar'])))
{
$fechadescuento=convertir_fecha($_POST['fechadelpago']);		// revisar que no hayan nominas con esa fecha
for ($if = 1; $if <= $_POST['nrosemanas']; $if++)
{
	if ($if > 1)
	{
		$sql = "select date_add('".$fechadescuento."', interval 7 day) as fechadescuento";
...
//			$sql_200="select cod_prof, ced_prof, concat(ape_prof, ' ', nombr_prof) as nombre, ctan_prof from sgcaf200 where (ucase(statu_prof) != 'RETIRA') and (tipo_socio='P') and (ced_prof='V-18785684' or ced_prof='V-09553047') order by ced_prof";
...
//			echo $sql_200;
			...
			while ($r200 = mysql_fetch_assoc($a_200))
			{
				...
				$sql_310="select * from sgcaf310 where (stapre_sdp='A') and (codpre_sdp='004') and (cedsoc_sdp='$micedula') and (f_1cuo_sdp <= '$fechadescuento') order by codpre_sdp";
...
//				if ((mysql_num_rows($a_310)) > 0) echo $sql_310;
				...
				revisar_prestamo($r200,$a_310,$a_360,$fechadescuento,$micedula,$ip,$gestor,$if);
...
*/
	...
	echo "<form action='frm25.php?accion=AsientoContable' name='form1' method='post'>";

	echo '</div>';
}	// ($accion=='ImpresionListados') 


if (($accion=='ListadoDeCuotas') and (($_POST['descontar']== 'on'))) 
{
//	echo 'descontar '.$_SESSION['descontar']; 
	$fechadescuento=$_POST['fechadelpago'];
	echo "<form action='frm25.php?accion=AsientoContable' name='form1' method='post'>";
	$_SESSION['descontar']='S';
	$hoy = date("d/m/Y");
    $fechanueva=explode('/',$hoy);
...
if (($accion=='AsientoContable')) { // and ($nominasnormales == 'on')) {
	// echo 'fechas' .$_POST['fechadescuento']. ' ' .$_POST['fechaasiento'];
	if ($_SESSION['descontar']=='S')
		QuienesDesconte($_POST['fechadescuento'], $_POST['fechaasiento']);
//		AsientoDescuento($fechadescuento);
	else 
	{ 
		AsientoAbono($fechadescuento, $_POST['fechaasiento']);
...
function AsientoDescuento($fechadescuento)
{
	...
	$sql_360="select * from sgcaf360 where (cod_pres='004')";
	$a_360=mysql_query($sql_360);
...
function AsientoAbono($fechadescuento, $fechaasiento)
{
	...
	$fechaasiento=convertir_fecha($fechaasiento);
...
//		echo $header[$col_listado];
	...
	$asiento=$fechaasiento; // $_POST['$fechadescuento'];
	$asiento=explode('-',$asiento);
	$asiento=$asiento[0].$asiento[1].$asiento[2];
//	echo 'el asiento '.$asiento;
	$b=$fechaasiento;
...
function revisar_prestamo($r200,$a_310,$a_360,$fechadescuento,$micedula,$ip,$gestor, $if)
{
	$primeravez=0;
	$totalxsocio=0;
	while ($r310 = mysql_fetch_assoc($a_310))
	{
		if (! $r310['renovado'])
			if ($r310['stapre_sdp'] == 'A')
				acumular($r200,$r310,$a_360,$fechadescuento,$micedula,$primeravez,$ip,$totalxsocio, $if) ;
			else ;
		else 
			if ($r310['stapre_sdp'] == 'A')
...
function acumular($r200,$r310,$a_360,$fechadescuento,$micedula,&$primeravez,$ip,&$totalxsocio, $if)
{
	$minimacuota = 200000;  // 100; // 1; // 100000;
	if ($r310['codsoc_sdp'] =='01954')
		$minimacuota = 300000;  // 100; // 1; // 100000;

	$semanaadescontar=($if-1)*$minimacuota;
	// echo '<br>nro semana '.$if. ' cedula '.$micedula.'saldo '.($r310['monpre_sdp']-$r310['monpag_sdp']).' semana a descontar'.$semanaadescontar;
	$lacuota = $r310['monpre_sdp']-$r310['monpag_sdp'];
	$lacuota-=$semanaadescontar;
//	echo $lacuota;
	if ($lacuota > $minimacuota )
	{
		$lacuota = $minimacuota;
	}
	if ($lacuota < 0)
		$lacuota = 0;
	// echo 'cuota enviar '.$lacuota;
	if ($lacuota > 0)
	if ($_SESSION['descontar']== 'S')
	{
		$actualiza="update sgcaf310 set cuota_ucla=".$lacuota.", cuota = ".$lacuota." where registro =".$r310['registro'];
//  echo $actualiza;
		$ractualiza=mysql_query($actualiza);
		// $lacuota = $lacuota ; // $r310['cuota'];
	}
	else {
...
function QuienesDesconte($fechadescuento, $fechaasiento)
{
	$fechadescuento=$fechadescuento; // $fechadescuento; // $_POST['$fechadescuento'];
	$fechadescuento=explode('/',$fechadescuento);
	$fechadescuento=$fechadescuento[2].'-'.$fechadescuento[1].'-'.$fechadescuento[0];
	echo "<form enctype='multipart/form-data' action='frm25.php?accion=AsientoDescuento' name='form1' id='form1' method='POST' >";
	echo '<input type="hidden" name="fechadescuento" value="'.$fechadescuento.'">';
	echo '<input type="hidden" name="fechaasiento" value="'.convertir_fecha($fechaasiento).'">';
//	$fechassiento=$_POST['fechaasiento'];
//	$sql_360=$_SESSION['albanco'];
	$sql_360="select * from sgcaf360 where (cod_pres='004')";
	$a_360=mysql_query($sql_360);
	$rpl=300; 	// registros por listado
