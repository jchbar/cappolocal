...
$sql_nopr=$condicion_sql." from sgcanoprvol where ('$fechadescuento' = fecha) order by cedula "; // limit 0,500";
// echo $sql_nopr;
...
for($i=4;$i<count($header)-1;$i++){
	...
	$pdf->Cell($w[$micolumna],$alto,$header[$i],1,0,'C',1);
	$micolumna++;
	if ($micolumna > 9) {
		$linea+=5;
		$micolumna=1;
...
$pdf->SetY($linea);
$pdf->SetX($p[10]);
$pdf->Cell($w[10],$alto,$header[$i],1,0,'C',1);
