...
if ($accion=='ProcesarArchivos') {
	...
	*/
	...
	$nombre_archivo = 'ahorro_voluntario/'.$fechaarchivo.'domiciliacion.txt';
	$contenido = $nombre;
	...
	echo '<table>';
		echo '<tr>';
			echo '<td>';
				echo "<h2><a target=\"_blank\" href=\"retencionvoluntario.php?fechaaporte=$fecha_nomina\" onClick=\"info.html\', \'\',\'width=250, height=190\')\">Reporte Retenciones</a><br></h2>"; 
			echo '</td>';
			echo '<td>';
				...
				echo '<input type="submit" name="procesar" value="Descargar Archivo Ahorro Voluntario'.$nombre_archivo.'" />';
				echo '</form>';
			echo '</td>';
		echo '</tr>';
	echo '</table>';
...
function procesar_por_cobrar($fecha_nomina, $gestor)
{
	...
	$monto = $rv['montoVoluntario'];
	$comisiongastos = $rv['comisionVoluntario'];
	$emonto = $ecomision = 0;

	$sql = "select * from sgcaf200 where AhorroVoluntario = 'Si'";
	$resultado=mysql_query($sql);
	while ($rsql = mysql_fetch_assoc($resultado)) {
		$codigo=($rsql['cod_prof']);
		$cedula=($rsql['ced_prof']);

		$sql2="insert into t_his200 (cod_prof, hab_prof, hab_ucla, hab_voluntario, fecha, ip, proceso, cedula, pertenece, comision) values ('".$codigo."', 0, 0, ".$monto.", '".$fecha_nomina."', '".$ip."', now(), '".$cedula."', 'AV', ".$comisiongastos.")";
		$emonto+=$monto;
		...
		listadotxt($rsql,$rv['retencionVoluntario'],$gestor);
	...
	$desc='Nomina x Ahorro Voluntario del '.$fn;
	$explicacion=$desc;
...
?>
	...
	<div id='div1'>
		<form action='gnav.php?accion=ProcesarArchivos' name='form1' method='post' enctype='multipart/form-data' onsubmit='return realizar_abono(form1)'>
			<fieldset>
				...
				<table>
					...
					<tr>
						<td>
							Monto Bs. Ahorro Voluntario
						</td>
						<td> 
							<?php echo number_format($r['montoVoluntario'],$_SESSION['deci'],'.',',') ?>
						</td>
					<td>
					<tr>
						<td>
							Monto Bs. Comision Bancaria
						</td>
						<td> 
							<?php echo number_format($r['comisionVoluntario'],$_SESSION['deci'],'.',',') ?>
						</td>
					<td>
					<tr>
						<td>
							Monto Bs. Total Descuento Ahorro Voluntario
						</td>
						<td> 
							<?php echo number_format($r['retencionVoluntario'],$_SESSION['deci'],'.',',') ?>
						</td>
					<td>
