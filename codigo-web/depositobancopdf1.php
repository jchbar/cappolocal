...
// $sql_360="select * from sgcaf360 where (dcto_sem = $dcto) order by cod_pres"; //  limit 30"; //  limit 20";
...
$sql_nopr=$condicion_sql." from sgcanopr where ('$fechadescuento' = fecha) order by nombre";
// echo $sql_nopr;
...
//		$eitem=$$item;
		...
		$pdf->Cell($w[4],$alto,number_format($monto_actual,2,".",","),0,0,'R',0);
		if ($posicion > 8) {
			$posicion=3;
			$linea+=$salto;
		}
...
/*
	$pdf->Cell($w[4],$alto,number_format($r_nopr["colpre1"],2,".",","),0,0,'R',0);
	$pdf->Cell($w[5],$alto,number_format($r_nopr["colpre2"],2,".",","),0,0,'R',0);
	$pdf->Cell($w[6],$alto,number_format($r_nopr["colpre3"],2,".",","),0,0,'R',0);
	$pdf->Cell($w[7],$alto,number_format($r_nopr["colpre4"],2,".",","),0,0,'R',0);
	$pdf->Cell($w[8],$alto,number_format($r_nopr["colpre5"],2,".",","),0,0,'R',0);
	$pdf->Cell($w[9],$alto,number_format($r_nopr["colpre6"],2,".",","),0,0,'R',0);
	$t1=$r_nopr["colpre1"]+$r_nopr["colpre2"]+$r_nopr["colpre3"]+$r_nopr["colpre4"]+$r_nopr["colpre5"]+$r_nopr["colpre6"];
*/
	$pdf->SetX($p[10]);
	...
	if ($linea>=180)
		$linea=encabeza_l_prestamos($header,$w,$p,$pdf,$salto,$alto,$fechadescuento);
/*
	if ($crl > $rpl) {	// abro un nuevo reporte
		$crl=0;		// contador de registros por listado
		$pdf->Output();
		$pdf=new PDF('L','mm','Letter');
		$pdf->Open();
...
$pdf->SetY($linea);
$pdf->SetX(0);
$pdf->MultiCell(0,0,"Deposito de Pr�stamos al ".convertir_fechadmy($fechadescuento),0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',7);
$linea+=5;
$pdf->SetX(240);
