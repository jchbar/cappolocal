...
/*
	...
	header( "Content-Disposition: attachment; filename=".$_POST['archivosalida'].""); 
	$contenido = '';
*/
...
//	echo 'la salida'.$salida;
	$archivosalida=fopen ($salida, "w+");
//	generar_comprobante($ip,$archivosalida,$numerocuotas,&$contenido);
...
//		echo '<br><input type="text" name="totalgeneral" id="totalgeneral" value=0 readonly="readonly">';
	...
	if ((substr($datos[0],0,3)=='621') and (substr($datos[0],78,10)!='AUTORIZADO')) {
		...
		if (mysql_num_rows($result) < 1) {
			...
			echo "<input name='montos[]' type='hidden' value='".$row['cuota_ucla']*$numerocuotas."'>";
			echo "<strong>$codigo<br>$cedula</strong>";
			echo '<br>'.$socio['ape_prof'].' '.$socio['nombr_prof']."<br><strong>".number_format($monto,2,'.',',')."</strong>";
			$ti=0;
			$contador='cont_'.$codigo;
			$$contador=0;
			while ($row = mysql_fetch_assoc($resul2)) {
				$ti+=$row['cuota_ucla']*$numerocuotas;
				$$contador++;
			}
			...
			echo "</td><td rowspan='$filas' class='azul'>";
			echo '<br>Seleccionados <input type="text" name="D'.$codigo.'" id="D'.$codigo.'" value=0 readonly="readonly">';
			echo '<br>Diferencia <input type="text" name="R'.$codigo.'" id="R'.$codigo.'" value=0 readonly="readonly">';
			echo '</td></tr>';

			while ($row = mysql_fetch_assoc($resul2)) {
				echo '<tr><td>';
				echo $row['nropre_sdp'].'</td><td>'.$row['descr_pres'].'</td><td align="right">'.(number_format($row['cuota_ucla']*$numerocuotas,2,'.','.')).'</td>';
				echo '<td>';
//			echo $ti. ' '.$monto.'- '.($ti == $monto).'<br>';
				$interno++;
				$contadorgeneral++;
				echo '<input type="checkbox" name="';
				echo ''.$codigo.'_'.$interno;
//				echo 'arreglo';
				echo '" id="';
				echo ''.$codigo.'_'.$interno;
...
//					echo 'entre ';
				...
				echo ' onClick="calccanc(\''.$codigo.'\', '.$$contador.', '.$monto.')" > </td>';
				echo "<input type='hidden' name='V".$codigo.'_'.$interno."' id='V".$codigo.'_'.$interno."' value=".$row['cuota_ucla']*$numerocuotas.">";
				echo "<input type='hidden' name='montos".$contadorgeneral."' id='montos".$contadorgeneral."' value=".$row['cuota_ucla']*$numerocuotas.">";
//				echo "<input type='hidden' name='V".$codigo."' id='V".$codigo."' value=".$row['cuota_ucla']*$numerocuotas.">";
...
//		echo '></td></tr>' ;
			...
			echo "<input type='hidden' name='N".$codigo."' id='N".$codigo."' value=".$contador.">";
			echo "<input name='cuantos[]' type='hidden' value=".$$contador.">";
