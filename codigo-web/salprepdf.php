...
$sql_amor="select *,descr_pres, cuent_pres from sgcaf310, sgcaf360 where (stapre_sdp = 'A' and ! renovado) and (codpre_sdp=cod_pres) order by codpre_sdp, codsoc_sdp"; //   limit 30"; //  limit 20";
// echo $sql_amor;
...
$alto=3;
$salto=$alto;
$w=array(8,13,20,50,15,15,20,15,10); // ,25,25,25,25,25,25);
$p[0]=20;
...
//		echo $sql_200;
		...
		$pdf->SetX($p[5]);		$pdf->Cell($w[4],$alto,convertir_fechadmy($r_amor["f_1cuo_sdp"]),0,0,'LRTB');
		$pdf->SetX($p[6]);		$pdf->Cell($w[5],$alto,number_format($r_amor["monpre_sdp"]-$r_amor["monpag_sdp"],2,'.',','),0,0,'R');
		$pdf->SetX($p[7]);		$pdf->Cell($w[6],$alto,number_format($r_amor["cuota_ucla"],2,'.',','),0,0,'R');
		$pdf->SetX($p[8]);		$pdf->Cell($w[7],$alto,number_format($r_amor["ultcan_sdp"],0,'.',',').'/'.number_format($r_amor["nrocuotas"],0,'.',','),0,0,'R');
		$ultimo=$r_amor['otro_int'];
		$santerior+=($r_amor["monpre_sdp"]-$r_amor["monpag_sdp"]);
		$tcuota+=$r_amor["cuota_ucla"];

		$gsanterior+=($r_amor["monpre_sdp"]-$r_amor["monpag_sdp"]);
		$gtcuota+=$r_amor["cuota_ucla"];
	}
	else {
		$linea+=$alto;
		$pdf->SetY($linea);
...
////////////////////////////////////////////////////
function encabeza_l_prestamos($header,$w,$p,&$pdf,$salto,$alto,$fechadescuento,$nombreprestamo, $cuenta)
{
$pdf->AddPage();
$linea=25;
$pdf->SetY($linea);
...
$pdf->MultiCell(0,0,"Saldo de Prestamos al ".convertir_fechadmy($fechadescuento),0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',7);
$linea+=5;
$pdf->SetX(220);
