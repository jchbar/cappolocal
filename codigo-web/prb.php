...
?>
<script src="ajaxretucla.js" type="text/javascript"></script>
...
if (!$accion) {
	...
	$viejo=$viejo['viejos'];
	$sql='SELECT fecha, sum(hab_prof) as monto, count(fecha) as cuantos, pertenece FROM t_his200 where hab_prof > 0 and (fecha > "'.$viejo.'") and pertenece IS NOT NULL group by fecha desc, pertenece asc ';
	$a_sql=mysql_query($sql);
	// echo $sql; 
	echo "<form action='prb.php?accion=Abonar' name='form1' method='post' enctype='multipart/form-data' onsubmit='return realizar_abono(form1)'>";
	?>
	<fieldset><legend>Información Para Relacionar Retenciones</legend>
	<table>
	...
	// die('espero');
		...
		for ($i=0;$i<$registros;$i++)	// no es necesarios revisar el check si aparece es porq estan seleccionados para hacer el asiento 
		{
			...
			if (!empty($$variable)) 
			{
				...
				$sql="select * from t_his200 where (hab_prof > 0) and (fecha ='$fechanomina') and (pertenece = '$tipo') order by numreg"; 
				// echo $sql.'<br>';
				$a_amor=mysql_query($sql);
