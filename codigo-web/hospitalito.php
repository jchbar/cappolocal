...
if ((! $_POST['procesar']) and (! $_FILES['archivo']['name'])) {
	echo '<fieldset><legend>Informacion para procesar archivo para Hospitalito</legend>';
	echo '<form action="hospitalito.php" method="post" name="form1" enctype="multipart/form-data">';
	echo 'Fecha de Descuento Prestamo Hospitalito ';
?>
...
//	echo '<input type="checkbox" name="nominasemanal" value = "1" checked/> Nomina Semanal<br />';
	echo 'Indique el archivo (CSV) que contiene la informacion de Hospitalito<input name="archivo" type="file" value="Examinar"><br>';
...
$cuento='Cargo en Cuenta Prestamos Hospitalito de fecha '.$b;
...
//		$totalesgral+=($datos[3]*100)/100;
		...
		$hoy = date("Y-m-d");
		$sql_310="INSERT INTO sgcaf310 (codsoc_sdp, cedsoc_sdp, nropre_sdp, codpre_sdp, f_soli_sdp, f_1cuo_sdp, ultcan_sdp, monpre_sdp, stapre_sdp, cuota, nrocuotas, cuota_ucla, ip, interes_sd) VALUES ('".$r200['cod_prof']."', '".$micedula."', '$elnumero', '065', '$hoy', '$b', 0, $monto, 'A', $cuota, '$nrocuota', $cuota, '$ip', '$interes')";
//		echo $sql_310.'<br>';
		$saldo=$monto;
...
//			echo $sql810;
	...
	agregar_f820($asiento, $b, '-', $cuenta1, "Relacion Hospitalito de Fecha ".$b, $debe, $haber, 0,$ip,0,$referencia,'','S',0); 
}
