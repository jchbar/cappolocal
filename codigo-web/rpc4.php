	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$filtro="SELECT substr(cta_contab,18,4) as cta_contab,descrip, motivodes FROM sgcaf610 WHERE ((substr(cta_contab,1,17)='".$_SESSION['nro']."') and ((substr(cta_contab,18,21) LIKE '$queryString%') or (descrip LIKE '$queryString%'))) and motivodes='' order by cta_contab LIMIT 10";
			//    echo $filtro; 
