	...
	if(!$db) {
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) >0) {
				...
				$filtro="SELECT substr(cue_codigo,18,4) as cue_codigo,cue_nombre FROM sgcaf810 WHERE ((substr(cue_codigo,1,17)='".$_SESSION['nro']."') and ((substr(cue_codigo,18,21) LIKE '$queryString%') or (cue_nombre LIKE '$queryString%'))) and (cue_nivel = '".$niveles ."') order by cue_codigo LIMIT 10";
			    //echo $filtro; 
