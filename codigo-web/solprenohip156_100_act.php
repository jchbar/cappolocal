...
if ($prestamo['aprobar']==0) {
	...
	$cuento.=' Dicho préstamo comenzará a ser descontado a partir del '.convertir_fechadmy($prestamo['fecha_acta']);
}
else
{
	$cuento= '   Yo, '.trim($socio['ape_prof']). ' '.trim($socio['nombr_prof']).' mayor de edad, titular de la Cédula de Identidad: ';
	$cuento.=$socio['ced_prof'].' y Socio de esta institución identificado con el Código '.$socio['cod_prof'];
//	$cuento.=' me dirijo a la Junta Directiva con el fin de solicitar la cantidad de Bolívares '.strtoupper(num2letras($prestamo['monpre_sdp']-$prestamo['inicial']));
	$cuento.=' me dirijo a la Junta Directiva con el fin de solicitar la cantidad de Bolívares '.strtoupper('CIEN CINCUENTA MIL ');
//	$cuento.='  ****Bs. ('.trim(number_format($prestamo['monpre_sdp']-$prestamo['inicial'],2,".",",")).')**** en calidad de prestamo. ';
	$cuento.='  ****Bs. ('.trim(number_format(150000,2,".",",")).')**** en calidad de prestamo. ';
	$cuento.='De igual manera, en reunión de Junta Directiva según Acta N° ' . $prestamo['nro_acta'] . ' de fecha ' . convertir_fechadmy($prestamo['fecha_acta']).' se acordó la aprobación del prestamo arriba indicado al socio ';
...
//	$cuento.='  ****Bs. ('.trim(number_format($prestamo['monpre_sdp']-$prestamo['inicial'],2,".",",")).')****. ';
	$cuento.=' por la cantidad de Bolívares CIEN CINCUENTA MIL ';
	$cuento.='  ****Bs. (150.000,00)****. ';
...
$pdf->SetY($linea);
$pdf->SetX($columna[0]);
$pdf->Cell(0,6,'Amortizacion de intereses s/Prestamos ',0,0,'L',0);
$pdf->SetX($columna[2]);

$sqlgiros="select * from sgcafgir where activo=1 and tipo='Interes'";
...
while($r_giros=mysql_fetch_assoc($a_giros)) {
	...
	$pdf->Cell(60,6,number_format($monto_giros,2,".",","),0,0,'R',0);
	$t_deduccion+=$monto_giros;
}
...
$pdf->SetY($linea);
$pdf->SetX($columna[0]);
$pdf->Cell(0,6,'GIROS A DESCONTAR',0,0,'L',0);

$linea+=5;
$pdf->SetY($linea);
$pdf->SetX($columna[0]);
$pdf->Cell(0,6,'1. Cuotas especiales (Giros) sobre prestamos',0,0,'L',0);
$pdf->SetX($columna[2]);
...
while($r_giros=mysql_fetch_assoc($a_giros)) {
	...
	$monto_giros=($ncuotas*$cuota_giros);
	$pdf->Cell(60,6,number_format($monto_giros,2,".",","),0,0,'R',0);
	$t_reintegro+=$monto_giros;
}

/*
$pdf->Cell(60,6,number_format(4770,2,".",","),0,0,'R',0);
$t_reintegro+=4770;
*/
$linea+=5;
$pdf->SetY($linea);
$pdf->SetX($columna[0]);
$pdf->Cell(0,6,'2. Amortizacion de Capital',0,0,'L',0);
$pdf->SetX($columna[2]);

$sqlgiros="select (sum(monto*parte)) as tgiros from sgcafgir where activo=1 and tipo='Capital'";
$a_giros=mysql_query($sqlgiros);
$losgiros = 0;
while($r_giros=mysql_fetch_assoc($a_giros)) {
	$cuota_giros=1;
	$ncuotas=$r_giros['tgiros'];
	$monto_giros=($ncuotas*$cuota_giros);
	$pdf->Cell(60,6,number_format($monto_giros,2,".",","),0,0,'R',0);
	$t_reintegro+=$monto_giros;
}
...
$pdf->Cell(60,6,number_format(29700,2,".",","),0,0,'R',0);
$t_reintegro+=29700;
*/
$linea+=5;
$pdf->SetY($linea);
...
$pdf->SetX($columna[2]);
$pdf->Cell(60,6,'Neto a Depositar',0,0,'R',0);
$pdf->SetX($columna[3]);
...
$pdf->SetFont('Arial','B',10);
$pdf->SetX($columna[0]);
$cuentogiros='Observacion: EMISION DE GIROS CON DESCUENTOS ESPECIALES ASI: ';
$sqlgiros="select * from sgcafgir where activo=1";
$a_giros=mysql_query($sqlgiros);
$losgiros = 0;
while($r_giros=mysql_fetch_assoc($a_giros)) {
	...
	$ncuotas=$r_giros['parte'];
	$monto_giros=($ncuotas*$cuota_giros);
	$cuentogiros.=' Bs. '.number_format($monto_giros,2,'.',',').' Vence el '.$primer_dcto.' / ';
}
//$pdf->Cell(0,6,$cuentogiros,0,0,'C',0);
$pdf->MultiCell(0,5,$cuentogiros,0,'L'); 
$pdf->SetFont('Arial','',8);
...
recuerden pasar la informacion referente a los indomiciliados y pasar
igualmente la informacion de los socios que hayan enviado a descuento y
los mismos se hayan ejecutado correctamente para sacarlos de los bloqueos
*/
?>
