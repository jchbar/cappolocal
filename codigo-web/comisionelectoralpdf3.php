...
$sql="select (@a:=@a+1) as linea, escuela, cod_prof, ced_prof, nombre, ubicacion from (";
$sql.="SELECT @a :=0, ubicacion as escuela, cod_prof, cedula as ced_prof, nombre, ubicacion FROM listado, sgcaf200 where (cedula = ced_prof) and (";
	$sql.='UPPER(ubicacion)!="" ';
// $sql.='(tipo_socio = "P" or tipo_socio = "E") ';
...
$alto=5;
$salto=$alto;
$w=array(20,20,30,80, 20); // ,15,50,15,15); 
$p[0]=10;
...
//		if ($np==$rsocio['escuela']) {
			...
			$pdf->SetX($p[4]);		$pdf->Cell($w[4],$alto,$rsocio["ubicacion"],0,0,'L');
//		}
