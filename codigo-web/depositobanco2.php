...
if (!$accion) {
	...
	echo '<fieldset><legend>Informaci�n Para Deposito de Prestamos</legend>';
...
// desactivacion de 18 a�os pa' tras
...
*/
	...
	echo '<option value="032">032 - LIBRERIA QUINCENAL</option>'; 
	...
	echo '<option value="072">072 - OPTICA (Anual)</option>'; 
	echo '<option value="074">074 - NO HIPOTECARIO</option>'; 
	echo '<option value="075">075 - NO HIPOTECARIO</option>'; 
...
//	echo "<input type = 'submit' value = 'Nuevo Prestamo'></form>\n"; 
	...
	echo '<fieldset><legend>Atentos con esta informaci�n</legend>';
	...
	if (is_writable($nombre_archivo)) {
		...
		if (!$gestor = fopen($nombre_archivo, 'a')) {
			...
			if ($estatutarios == 'on')
			{
				...
				$sql_360="select * from sgcaf360 where (dcto_sem = 1) and (cod_pres <> '032' and cod_pres <> '055' and cod_pres <> '064' and cod_pres <> '066' and cod_pres <> '067' and cod_pres <> '068' and cod_pres <> '072' and cod_pres <> '065')  and (tipo != 'VOLUNTARIO') order by cod_pres";
				$_SESSION['tipo']='Prestamos Estatutarios ';
				$_SESSION['codigoprestamo']='';
			}
			else {
				...
				$_SESSION['codigoprestamo']=$elprestamo;
			}
//			echo $sql_360;
...
//				if ((mysql_num_rows($a_310)) > 0) echo $sql_310;
				...
				echo "<script>callprogress(".round($porcenta).")</script>"; //llamo a la funci�n JS(JavaScript) para actualizar el progreso
...
//	echo 'descontar '.$_SESSION['descontar']; 
	if ($_SESSION['descontar']=='S')
		QuienesDesconte($fechadescuento, $_POST['fechaasiento']);
...
// echo 'coigo ';
// .$_SESSION['codigoprestamo'] ;
	if ($_SESSION['codigoprestamo'] == '046') // funeraria
	{

		$comisionbancaria = $totalxsocio * (7/100);
...
//		echo $lacuota.'<br>';
	...
	{
		$posicion++;
			//echo $r310['nropre_sdp'].' - ' .$r310['codpre_sdp'].' '.$r360['cod_pres'].'<br>'; // . ' - '.$r360['cod_pres'];
