...
if (!$accion) {
	...
	$viejo=$viejo['viejos'];
	$sql='SELECT fecha, sum(cuota) as monto, count(fecha) as cuantos FROM sgcaamor where proceso = 1 and semanal = 1 and (fecha > "'.$viejo.'") group by fecha desc';
	$a_sql=mysql_query($sql);
//	echo $sql; 
...
//			echo 'destino '.$destino.'<br>';
	...
	for ($i=0;$i<$registros;$i++)	// no es necesarios revisar el check si aparece es porq estan seleccionados para hacer el asiento 
	{
		...
		if (!empty($$variable)) 
		{
			...
			$sql="select nropre from sgcaamor where fecha ='$fecha' and proceso = 1 and (semanal = 1) order by codsoc"; // limit 10";
//			echo $sql.'<br>';
...
//				for ($j=3;$j<5;$j++) { // para prueba eliminar
					...
					$sql="select * from sgcaamor where ((fecha ='$fecha') and (proceso = 1) and (semanal = 1) and (tipo ='$aprocesar')) order by codsoc"; // limit 10";
//					echo $sql.'<br>';
...
*/
					...
					$sql="update sgcaf310 set stapre_sdp='C', renovado=1 where (codpre_sdp='046' and ultcan_sdp=nrocuotas)";
					if (! mysql_query($sql))
							echo $sql.'<br>';
...
*/
		...
		$sql="select *, date_add('$fecha',INTERVAL 90 DAY) as suspension from sgcaamor where fecha ='$fecha' and proceso = 1 and (semanal = 1) order by codsoc"; 
