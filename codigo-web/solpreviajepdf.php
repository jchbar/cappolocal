...
$sql = "select *, sum((cuota)*nrocuotas) as montoprestamo from sgcaf310, sgcaf360 where (cedsoc_sdp='$micedula') and (codpre_sdp=cod_pres) and (codpre_sdp='039') and (stapre_sdp='A') group by codpre_sdp order by f_1cuo_sdp ";
...
if ($prestamo['aprobar']==0) {
	...
	$cuento.=' me dirijo a la Junta Directiva con el fin de solicitar la cantidad de Bolívares '.strtoupper(num2letras($prestamo['montoprestamo']-$prestamo['inicial']));
	$cuento.='  ****Bs. ('.trim(number_format($prestamo['montoprestamo']-$prestamo['inicial'],2,".",",")).')**** en calidad de prestamo. ';
...
else
{
	...
	$cuento.=' por la cantidad de Bolívares '.strtoupper(num2letras($prestamo['montoprestamo']-$prestamo['inicial']));
	$cuento.='  ****Bs. ('.trim(number_format($prestamo['montoprestamo']-$prestamo['inicial'],2,".",",")).')****. ';
...
$pdf->SetX($columna[3]);
$pdf->Cell(60,6,number_format($prestamo['montoprestamo'],2,".",","),0,0,'R',0);
$linea+=5;
$pdf->SetY($linea);
$pdf->SetX($columna[0]);
...
$pdf->SetX($columna[3]);
$pdf->Cell(60,6,number_format(($prestamo['montoprestamo']-$prestamo['inicial']),2,".",","),0,0,'R',0);
$pdf->SetFont('Arial','',12);
...
$pdf->SetX($columna[3]);
$neto=($prestamo['montoprestamo']-$prestamo['inicial'])+$t_reintegro-$t_deduccion;
...
$pdf->SetFont('Arial','B',10);
$pdf->SetX($columna[0]);
$pdf->Cell(0,6,'Condiciones del Crédito / Observaciones ',0,0,'C',0);
$pdf->SetFont('Arial','',8);
...
$cuento.=' *** Horario: Ingreso Hotel '.$_SESSION['viaje'].' Check-in 03:00pm Check-out 1:00pm / NO HAY CAMBIOS DE FECHA / NO INCLUYE TRANSPORTE Barquisimeto - Morrocoy - Barquisimeto,el transporte corre por cuenta propia del titular. Incluye: Hospedaje / Comidas y Refrigerios / Bebidas Nacionales Alcoholicas y no alcoholicas / Traslado Cayo Boca Seca '.$_SESSION['observa'].' ** ESTOS PRESTAMOS NO TIENEN DERECHO A REINTEGRO *** ';
$pdf->MultiCell(0,5,$cuento,0,'L'); 
...
$linea+=25;
$columna=array(10,30,110,140);
$pdf->SetY($linea);
$pdf->SetX($columna[0]);
$pdf->Cell(0,6,'Cedula',0,0,'L',0);
$pdf->SetX($columna[1]);
$pdf->Cell(60,6,'Nombre',0,0,'L',0);
$pdf->SetX($columna[2]);
$pdf->Cell(60,6,'Nacimiento',0,0,'L',0);
$sql_deducciones="select * from sgcaviaj where cedtitular='$micedula' and fecviaje = '".$_SESSION['viaje']."'";
$a_deduccion=mysql_query($sql_deducciones);
while($r_deduccion=mysql_fetch_assoc($a_deduccion)) {
	$linea+=5;
	$pdf->SetY($linea);
	$pdf->SetX($columna[0]);
	$pdf->Cell(0,6,$r_deduccion['cedbene'],0,0,'L',0);
	$pdf->SetX($columna[1]);
	$pdf->Cell(60,6,$r_deduccion['nombre'],0,0,'L',0);
	$pdf->SetX($columna[2]);
	$pdf->Cell(60,6,$r_deduccion['fecnac'],0,0,'L',0);
}
