...
if ($accion == "Renovar") {	// seleccionar el tipo de prestamo nuevo de renovacion
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='zapatosm.php?accion=Solicitar' name='form1' id='form1' method='post' onsubmit='return valpre(form1)'";
...
// 		bucle de listado
		...
		echo "<td class='centro'><a href='zapatosm.php?accion=Ver&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/page_user_dark.gif' width='16' height='16' border='0' title='Consultar' alt='Consultar'/></a></td>";
		echo "<td class='centro'>";
		if ($row['renovacion']>1)
			if ($row['ultcan_sdp'] >= $row['renovacion']) {
				echo "<a href='zapatosm.php?accion=Renovar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
				...
				echo "<a href='zapatosm.php?accion=ReAjustar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
...
if (!$accion) {
	echo "<form action='zapatosm.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
...
if (($accion == "Editar") or ($accion=="Renovar")) {	// muestra datos para prestamo
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='zapatosm.php?accion=EscogePrestamo' name='form1' method='post' onsubmit='return valsoc(form1)'>";
...
if ($accion == "ReAjustar") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='zapatosm.php?accion=Reajuste' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
...
// phpinfo();
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='zapatosm.php?accion=Concretar' name='form1' id='form1' method='post' onsubmit='return valpreres(form1)'";
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="zapatosm.php?accion=Buscar">';
...
function solicitar_fiadores($elnumero,$lacedula)
{
	...
	echo "<form enctype='multipart/form-data' action='zapatosm.php?accion=Fiadores' name='form1' id='form1' method='POST' onsubmit='return valfiadores(form1)'>";
...
//			echo '<td class="centro azul"><input type="checkbox" id="eliminar'.$registros.'" name="eliminar'.$registros.'" value='.$r_320["registro"] .' onClick="eliminafianza()"> </td>' ;
		...
		echo "<a href='zapatosm.php?accion=Eliminar&cedula=".$lacedula."&nropre=".$elnumero."&registro=".$r_320['registro']."& '  onClick='return conf_elim_fiadores()'>";
...
//	$elnumero=substr($elnumero['nropre_sdp'],5,3);
	...
	$sql_zapa="select * from sgcazapa where categoria='zapatoM' order by modelo";
...
*/
	...
	echo '<input type="button" name="calculo" value="Calcular Cuota" onClick="ajax_call_zapatosm()">	';
