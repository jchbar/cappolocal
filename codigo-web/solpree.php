...
if ($accion == "Renovar") {	// seleccionar el tipo de prestamo nuevo de renovacion
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=Solicitar' name='form1' id='form1' method='post' onsubmit='return valpre(form1)'";
...
if ($accion == 'Buscar')  {
	...
	// if (($_SERVER['REMOTE_ADDR']!='192.168.100.62') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.96') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.3') AND ($_ENV['COMPUTERNAME']!='JCHB-DM1'))	// icono para cancelar prestamo
		if (($_SERVER['REMOTE_ADDR']!='192.168.100.64') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.96') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.3') AND ($_ENV['COMPUTERNAME']!='JCHB-DM1'))	// icono para cancelar prestamo
...
// 		bucle de listado
		...
		echo "<td class='centro'><a href='solpree.php?accion=Ver&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/page_user_dark.gif' width='16' height='16' border='0' title='Consultar' alt='Consultar'/></a></td>";
		echo "<td class='centro'><a href='solpree.php?accion=ModificarCuota&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/modiftbl.jpg' width='16' height='16' border='0' title='Modificar Cuota' alt='Modificar Cuota'/></a></td>";
		echo "<td class='centro'>";
		if ($row['renovacion']>1)
			if ($row['ultcan_sdp'] >= $row['renovacion']) {
				echo "<a href='solpree.php?accion=Renovar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
				...
				echo "<a href='solpree.php?accion=ReAjustar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
			...
			// if (($_SERVER['REMOTE_ADDR']=='192.168.100.4') OR ($_SERVER['REMOTE_ADDR']=='192.168.1.96') OR ($_SERVER['REMOTE_ADDR']=='192.168.100.62') or ($_ENV['COMPUTERNAME']=='JCHB-DM1'))	// icono para cancelar prestamo
			//	jc / mendez viejo / mendez nuevo 
			{
				echo "<a href='solpree.php?accion=CancelarPrestamo&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
			...
			// esta es la linea 
			if (($_SERVER['REMOTE_ADDR']=='192.168.100.4') OR ($_SERVER['REMOTE_ADDR']=='192.168.100.64') or ($_ENV['COMPUTERNAME']=='JCHB-DM1')) // icono para modificar neto a depositar prestamo
			{
//				echo "<td class='centro'>";
				echo "<a href='solpree.php?accion=NetoDepositar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
...
if (!$accion) {
	echo "<form action='solpree.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
...
if (($accion == "Editar") or ($accion=="Renovar")) {	// muestra datos para prestamo
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=EscogePrestamo' name='form1' method='post' onsubmit='return valsoc(form1)'>";
...
if ($accion == "ReAjustar") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=Reajuste' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
...
if ($accion == "ModificarCuota") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=ModificaCuota' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
// phpinfo();
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=Concretar' name='form1' id='form1' method='post' onsubmit='return valpreres(form1)'";
...
if ($accion == "CancelarPrestamo") {	//  para cancelar prestamos
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=CancelaPrestamo' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
if ($accion == "NetoDepositar") {	//  para modificar neto a depositar de los prestamos
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=NetoDeposito' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="solpree.php?accion=Buscar">';
...
function solicitar_fiadores($elnumero,$lacedula)
{
	...
	echo "<form enctype='multipart/form-data' action='solpree.php?accion=Fiadores' name='form1' id='form1' method='POST' onsubmit='return valfiadores(form1)'>";
...
//			echo '<td class="centro azul"><input type="checkbox" id="eliminar'.$registros.'" name="eliminar'.$registros.'" value='.$r_320["registro"] .' onClick="eliminafianza()"> </td>' ;
		...
		echo "<a href='solpree.php?accion=Eliminar&cedula=".$lacedula."&nropre=".$elnumero."&registro=".$r_320['registro']."& '  onClick='return conf_elim_fiadores()'>";
