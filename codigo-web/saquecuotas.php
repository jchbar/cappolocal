...
// echo 'lllll';
if(isset($_GET['tipo'])){
	$comando="select n_cuo_pres, i_max_pres, tipo_interes, en_ajax from sgcaf360 where cod_pres='$tipo'";
	$resultado=mysql_query($comando);
	$rprestamo = mysql_fetch_assoc($resultado);
	$cuota=$rprestamo['n_cuo_pres'];
	$interes_sd=$rprestamo['i_max_pres'];
	$tipo_interes=$rprestamo['tipo_interes'];
	$calculo=$rprestamo['en_ajax'];
//	for ($i=1;$i<=$rprestamo['n_cuo_pres'];$i++
//      echo "obj.options[obj.options.length] = new Option($i,$i);\n";
  
header("Content-Type: text/xml");
echo '<?xml version="1.0" encoding="utf-8"?>';
echo "<resultados>";
echo utf8_encode("<ncuota>$cuota</ncuota>");		// sirve asi y como esta abajo tambien
//echo "<cuota>".$interes."</cuota>";
echo utf8_encode("<interes_sd>$interes_sd</interes_sd>");	
echo utf8_encode("<tipo_interes>$tipo_interes</tipo_interes>");	
echo utf8_encode("<calculo>$calculo</calculo>");	
echo "</resultados>";
