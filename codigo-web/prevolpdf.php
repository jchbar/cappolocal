...
if ($prestamo['aprobar']==0) {
	...
	$cuento.='  ****Bs. ('.trim(number_format($prestamo['monpre_sdp']-$prestamo['inicial'],2,".",",")).')**** en calidad de prestamo por Ahorros Voluntarios. ';
...
*/
   $cuento= '   Yo, '.trim($socio['ape_prof']). ' '.trim($socio['nombr_prof']).' mayor de edad, titular de la Cédula de Identidad: ';
   $cuento.=$socio['ced_prof'].' y Socio de esta institución identificado con el Código '.$socio['cod_prof'];
   $cuento.='. Por medio del presente recibo de esta institución la cantidad de Bolívares '.strtoupper(num2letras($prestamo['monpre_sdp']-$prestamo['inicial']));
   $cuento.='  ****Bs. ('.trim(number_format($prestamo['monpre_sdp']-$prestamo['inicial'],2,".",",")).')**** en calidad de prestamo por Ahorros Voluntarios. ';
   $cuento.=' En este sentido, las cuotas a descontar por el mencionado prestamo son (es) ' . trim($prestamo['nrocuotas']) . ' cuota(s) de Bs. ';
   $cuento.=strtoupper(num2letras($prestamo['cuota'])); 
   $cuento.=' Bs. ('.number_format($prestamo['cuota'],2,".",",").') ';
   $cuento.=' con el Bono de Guerra y/o Bono de Alimentacion cuya fecha de pago final es '.convertir_fechadmy3($prestamo['f_1cuo_sdp']).' dejando el dinero antes indicado en la cuenta Nro. '.$socio['ctan_prof'];
