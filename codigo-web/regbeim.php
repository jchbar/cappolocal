...
// SetAutoPageBreak();
//////////////////////// autoizacion de descuento ahorros estatutarios
$pdf->AddPage();
$pdf->SetX(0);
$salto = 5;
$linea=20;
$notas = 'NOTAS: a. El descuento de retenciones y/o préstamos se realizará el día de depósito de pago ó el día hábil siguiente bancario. b. En caso de INDOMICILIACION se suspenderá el derecho a préstamos por un lapso de seis (06) meses a partir de la próxima domiciliación satisfactoria';
for ($i=0; $i < 3 ; $i++) { 
  if ($i > 0)
  {
    $saltoanterior = $salto;
    ...
    $pdf->SetY($linea);
    $pdf->SetX(30);
    $pdf->Image('fpdf/logo/logo.jpg',10,$linea,20);

    $pdf->SetY($linea);
    $pdf->SetX(30);
    $pdf->SetFont('Arial','B',11);
    //  $this->MultiCell(180,4,trim($lafila["nombr_empr"]),0,'LC');//
    $pdf->Cell(180,4,trim($lafila["nombr1_empr"]),0,0,'C');//

    $linea+=$salto;
    $pdf->SetY($linea);
    $pdf->SetX(30);
    $pdf->Cell(180,4,trim($lafila["nombr2_empr"]),0,0,'C');//

    $linea+=$salto;
    $pdf->SetY($linea);
    $pdf->SetX(0);
    $pdf->SetFont('Arial','B',14);
    $pdf->Cell(180,0,trim($lafila["alias_empr"]),0,0,C);//
    $pdf->Ln();   
    $pdf->Ln(10);
    $salto=$saltoanterior;
  }
  ...
  $concepto='A';
  $pdf->SetFont('Arial','B',14);
  $pdf->MultiCell(0,0,"A U T O R I Z A C I O N   D E   D E S C U E N T O (Ahorro Estatutario)",0,C,0);
  $pdf->SetY($linea);
  ...
  $pdf->SetY($linea);
  $pdf->SetX(10);
  $pdf->Cell(0,0,'Firma Conforme',0,0,'L'); 
  // $linea+=$salto;
  $pdf->SetY($linea);
  $pdf->SetX(35);
  $pdf->Cell(0,0,'________________________',0,0,'L'); 
  // $linea+=$salto;
  $pdf->SetY($linea);
  $pdf->SetX(100);
  $pdf->Cell(0,0,'C.I.:___________________',0,0,'L'); 
  // $linea+=$salto;
  $pdf->SetY($linea);
  $pdf->SetX(10);
  ...
  $pdf->SetFont('Arial','',7);
  $pdf->SetX(10);
  $pdf->Cell(0,0,($i+1).' -> '.($i==0?'Expendiente':($i==1?'UCLA':'Socio')),0,0,'C'); 
  $pdf->SetFont('Arial','',10);
  $linea+=$salto;
  $pdf->SetY($linea);
  $pdf->SetX(10);
...
if ($socio['AhorroVoluntario'] == 'Si')
{
  ...
  for ($i=0; $i < 2 ; $i++) { 
    if ($i >= 0)
    {
      ...
      $consulta = "Select * from sgcaf100";
      $resultado = mysql_query($consulta); // , $conexion);
      $rempresa=mysql_fetch_assoc($resultado); 
      $filas=mysql_num_rows($resultado);

      $notas = 'NOTAS: a. El monto a Descontar por Ahorro Voluntario es de Bs. '.number_format($rempresa['retencionVoluntario'],2,".",",").'; de este se destinara Bs. '.number_format($rempresa['montoVoluntario'],2,".",",").' al Ahorro y Bs. '.number_format($rempresa['comisionVoluntario'],2,".",",").' el monto cobrado por el Banco por la gestion de domiciliacion. b. El descuento de retenciones y/o préstamos se realizará el día de depósito de pago y/ó hasta el tercer día continuo siguiente bancario. c. En caso de INDOMICILIACION se suspenderá el derecho a préstamos por un lapso de tres (03) meses a partir de la próxima domiciliación satisfactoria. d. El monto por concepto de Ahorro Voluntario se ajustara cuando sea necesario';
      ...
      if ($i > 0)
      {
          ...
          $pdf->SetFont('Arial','B',11);
          //  $this->MultiCell(180,4,trim($lafila["nombr_empr"]),0,'LC');//
          $pdf->Cell(180,4,trim($rempresa["nombr1_empr"]),0,0,'C');//

          $linea+=$salto;
          $pdf->SetY($linea);
          $pdf->SetX(30);
          $pdf->Cell(180,4,trim($rempresa["nombr2_empr"]),0,0,'C');//

          $linea+=$salto;
          $pdf->SetY($linea);
          $pdf->SetX(0);
          $pdf->SetFont('Arial','B',14);
          $pdf->Cell(180,0,trim($rempresa["alias_empr"]),0,0,C);//
          $pdf->Ln();$pdf->Ln(10);
          $salto=$saltoanterior;
          $linea+=$salto;
          $linea+=$salto;
    ...
    $concepto='A';
    $pdf->SetFont('Arial','B',14);
    $pdf->MultiCell(0,0,"A U T O R I Z A C I O N   D E   D E S C U E N T O (Ahorro Voluntario)",0,C,0);
    $pdf->SetY($linea);
    $pdf->SetFont('Arial','',10);
    $linea+=$salto;
    $pdf->SetY($linea);
    ...
    if ($enblanco)
        ...
        $pdf->Cell(100,0,replicate('_',25).' '.replicate('_',25),0,0,'L'); 
    $pdf->SetFont('Arial','',10);
    $pdf->SetX(130);
    $pdf->Cell(60,0,'mayor de edad, titular de la Cédula ',0,0,'L'); 
    $linea+=$salto;
    ...
    if ($enblanco)
        ...
        $pdf->Cell(20,0,replicate('_',10),0,0,'L'); 
    $pdf->SetFont('Arial','',10);
    $pdf->SetX(60);
    $pdf->Cell(85,0,'trabajador(a) Obrero(a) del Decanato o Dependencia, ',0,0,'L'); 
    $pdf->SetFont('Arial','BU',10);
    $pdf->SetX(145);
    if (!$enblanco)
        $eldecanato=replicate('_',20);
    $pdf->Cell(80,0,trim($eldecanato),0,0,'L'); 
    $pdf->SetX(180);
    $pdf->SetFont('Arial','',10);
    $linea+=$salto;
    $pdf->SetY($linea);
    ...
    $rempresa = mysql_fetch_assoc($query);
    */
    $cadena='de la '.trim($rempresa['patron_empr']).'. Autorizo al Consejo de Administración de '.trim($rempresa['nombr1_empr']).' '.trim($rempresa['nombr2_empr']).' ('.trim($rempresa['alias_empr']).'), para que proceda a descontar la cantidad de Bs.D '.number_format($rempresa['retencionVoluntario'],2,".",",").' de mi BONO DE ALIMENTACION (CESTA TICKET), por concepto de AHORRO VOLUNTARIO como socio de '.trim($rempresa['alias_empr']);
    ...
    $pdf->SetY($linea);
    $pdf->SetX(10);
    $pdf->Cell($extraer,0,substr($cadena,$extraer*3,$extraer),0,0,'L'); 

    /*
    for ($i=0; $i < strlen($cadena); $i+=100) { 
        $pdf->SetY($linea);
        $pdf->SetX(10);
        $pdf->Cell($extraer,0,substr($cadena,($i),$extraer),0,0,'L'); 
        $linea+=$salto;
         echo substr($cadena,($i),$extraer).'<br>';
    }
    */

    $te = 'email:______________________________________________________________________';
    $te.= 'WhatsApp:____________________________';
    // $linea+=$salto;
    $pdf->SetFont('Arial','I',8);
    $pdf->SetY($linea);
    $pdf->SetX(10);
    $pdf->MultiCell(0,4,$te,0,'L'); 
    ...
    if ($enblanco)
        ...
        $pdf->Cell(0,0,'Barquisimeto: ___/_____/_____','R',0,0); 
    $linea+=$salto;
    $pdf->SetY($linea);
    $pdf->SetFont('Arial','',7);
    $pdf->SetX(10);
    $pdf->Cell(0,0,($i+1).' -> '.($i==0?'Expendiente':($i==1?'Socio':'Socio')),0,0,'C'); 
    // $pdf->Cell(0,0,($i+1).' -> '.($i==0?'Expendiente':($i==1?'Socio':'Socio')),0,0,'C'); 
    $pdf->SetFont('Arial','',10);
    $linea+=$salto;
    $pdf->SetY($linea);
...
if ($socio['AyudaSolidaria'] != 'Si')
{
  ...
  for ($i=0; $i < 2 ; $i++) { 
    ...
    $concepto='A';
    $pdf->SetFont('Arial','B',14);
    $pdf->MultiCell(0,0,"A U T O R I Z A C I O N   D E  R E T I R O  (Ayuda Solidaria)",0,C,0);
    $pdf->SetY($linea);
    $pdf->SetFont('Arial','',10);
    $linea+=$salto;
    $pdf->SetY($linea);
    ...
    $rempresa = mysql_fetch_assoc($query);
    */
    $cadena='de la '.trim($rempresa['patron_empr']).'. Autorizo al Consejo de Administración de '.trim($rempresa['nombr1_empr']).' '.trim($rempresa['nombr2_empr']).' ('.trim($rempresa['alias_empr']).'), para que proceda a retirarme del Aporte de Ayuda Solidaria como socio de '.trim($rempresa['alias_empr']).' de esta misma manera, no podre solicitarla en caso de necesitar la misma';
...
function replicate($laultima,$digitos)
{
