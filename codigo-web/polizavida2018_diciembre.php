...
$cuento='Cargo Poliza Vida 2018-2019';
...
{
	if ($fila200['polizaactiva'] == 1)
	{
		echo $fila200['ced_prof']. ' - ' .$fila200['nombr_prof']. ' - ' .$fila200['ape_prof']. ' - ' .$fila200['edad']. ' - '.($fila200['edad']<$limite?$montoa:$montoj).'<br>';
		$monto=($fila200['edad']<$limite?$montoa:$montoj);
		$primerdcto=($fila200['edad']<$limite?$dctoa:$dctoj);
		$interes_sd=0;
		$debe=$monto;
		$monpre_sdp=$monto;
		$codsoc_sdp=$fila200['cod_prof'];
		...
		$lascuotas=1;
		$cuota=$monto / $lascuotas;
		$sql="insert into sgcaf310 (codsoc_sdp, cedsoc_sdp, nropre_sdp, codpre_sdp, f_soli_sdp, f_1cuo_sdp, monpre_sdp, monpag_sdp, nrofia_sdp, stapre_sdp, tipo_fianz, cuota, nrocuotas, interes_sd, cuota_ucla, netcheque, fecha_acta, ip, inicial, intereses, quien) values ('$laparte', '$micedula', '$elnumero','$elprestamo','$hoy', '$primerdcto', $monpre_sdp, 0, 0, '$estatus', '',$cuota, $lascuotas, $interes_sd, $cuota, $monpre_sdp, '$fechaacta', '$ip', $inicial, $intereses_diferidos, '".$_SERVER['REMOTE_ADDR']."')";
		if (!mysql_query($sql)) die ("<br>El usuario $usuario no tiene permiso para añadir prestamo.<br>".$sql.'<br>'.mysql_error());


		$cuenta1=$cuenta.'-'.$codigo;
		$totalesgral+=$debe;
		$referencia=$fila200['edad'];
		agregar_f820($asiento, $b, '+', $cuenta1, $cuento, $debe, $haber, 0,$ip,0,$referencia,'','S',0); 
	}
}
$debe=$totalesgral;
...
$referencia='';
agregar_f820($asiento, $b, '-', $cuenta1, $cuento, $debe, $haber, 0,$ip,0,$referencia,'','S',0); 
echo '<a href="exporte_poliza.php">Exportar archivo xls</a>';
...
*/
	...
	$nombre_archivo = 'nompre/'.$fechaarchivo.'polizavida2016.xls';
	$contenido = $nombre;
