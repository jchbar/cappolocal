...
$sql = "
	...
	$sql .= "ORDER BY ubicacion, nombre";
	// echo $sql;
$result_socios=mysql_query($sql) or die (mysql_error() . $sql);
...
$result_socios=mysql_query($sql) or die (mysql_error().$sql);
// echo 'registros '.mysql_num_rows($result_socios);
while ($row = mysql_fetch_assoc($result_socios))
{
	$codigo = $row['codigo'];
	// echo 'a'.$codigo;

	$sql = "select sum(mas_retencion) as mas_retencion, sum(mas_aporte) as mas_aporte from result_ahorro_est where codigo = '$codigo'";
	$est=mysql_query($sql) or die (mysql_error().$sql);
	...
	$ext=mysql_query($sql) or die (mysql_error().$sql);
...
// die(date('l jS \of F Y h:i:s A').'  procesar_prestamos ');
...
function pasar_a_cierre_historico_limpio($link, $fechaaporte)
{
	...
	$result=mysql_query($sql) or die (mysql_error().$sql);
	$sql = "select count(*) from historico_cierre_ahorros";
	$result=mysql_query($sql) or die (mysql_error().$sql);
	if (mysql_num_rows($result) < 1)
		die('<h2>No se guardo historico</h2>');
