...
$sql="select cod_prof, ced_prof, concat(trim(ape_prof),' ',nombr_prof) as nombre, ubic_prof, statu_prof, f_ret_capu from sgcaf200 where (upper(statu_prof) = 'RETIRA') order by ".$orden; //   limit 30"; //  limit 20";
// echo $sql;
...
while ($rsocio = mysql_fetch_assoc($asocio)){
	...
	$pdf->SetX($p[5]);		$pdf->Cell($w[5],$alto,convertir_fechadmy($rsocio["f_ret_capu"]),0,0,'C');
...
$pdf->MultiCell(0,0,"Listado de Socios Retirados al ".$hoy,0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',7);
$linea+=5;
$pdf->SetX(220);
