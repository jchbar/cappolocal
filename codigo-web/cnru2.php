...
function procesar_por_cobrar($archivo, $tipo, $proceso, $fecha_nomina, $gestor)
{
	...
	foreach ($lines as $line_num => $linea) {
		...
		$monto = $monto / 100;
		$lacomisiongastos = round($monto * ($comisiongastos / 100),1);
		$ecomision = $ecomision + $lacomisiongastos;
		...
		$sql2="insert into t_his200 (cod_prof, hab_prof, hab_ucla, fecha, ip, proceso, cedula, pertenece, comision) values ('".$codigo."', '".$monto."', '0', '".$fecha_nomina."', '".$ip."', now(), '".$cedula."', '".$tipo."', ".$lacomisiongastos.")";
		$emonto+=$monto;
		...
		listadotxt($r200,$lacomisiongastos,$gestor);
...
?>
	<div id='div1'>
		<form action='cnru2.php?accion=ProcesarArchivos' name='form1' method='post' enctype='multipart/form-data' onsubmit='return realizar_abono(form1)'>
			<fieldset>
				...
				<table>
					...
					<tr>
						...
						<td> 
							<input type="text" name="comisiongastos" id="comisiongastos" value="5.00">
						</td>
					<td>
