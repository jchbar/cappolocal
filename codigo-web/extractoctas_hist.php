...
if (! $elano)
{
	echo "<form enctype='multipart/form-data' method='post' name='form1'>Año a Consultar: ";
	$sqlanos="select * from histf835 order by ano desc";
	$resultado=mysql_query($sqlanos);
	echo '<select name="elano" size="1">';
	while ($fila2 = mysql_fetch_assoc($resultado)) {
		echo '<option value="'.$fila2['ano'].'">'.$fila2['ano'].'</option>'; 
	}
	echo '</select> *'; 
...
if (!$cuenta) {
	...
	echo '<table>';
	echo '<tr><td>';
	echo 'Año Seleccionado </td><td>';
	echo '<input type="text" size="4" tabindex="1" name="anosolicitado" id="anosolicitado" value ="'.$_POST['elano'].'"  readonly="readonly" />';
	echo '<input type="hidden" size="4" tabindex="1" name="elano" id="elano" value ="'.$_POST['elano'].'"  readonly="readonly" />';
	echo '</td></tr>';
//	echo 'Año Seleccionado </td><td>'.$_POST['elano'].'</td></tr>';
	$_SESSION['anohistorico']=$_POST['elano'];
	echo "<tr><td>Cuenta: </td><td>";
//	echo "<input type='text' name='cuenta' size='20' maxlength='20'> \n";
?>
	<input type="text" size="30" tabindex="1" name="cuenta" id="inputString" onKeyUp="lookuph(this.value);" onBlur="fillh();" value ="<?php echo $cuenta;?>" autocomplete="off"/>
...
<?php 
	...
	echo '<tr><td>';
	echo 'Fecha Inicio ';
	echo '<input type="text" size="10" tabindex="3" name="fechai" id="fechai" value ="'.$_POST['elano'].'-01-01"  readonly="readonly" />';
	echo '</td></tr>';
	echo '<tr><td>';
	echo 'Fecha Final';
	echo '<input type="text" size="10" tabindex="3" name="fechaf" id="fechaf" value ="'.$_POST['elano'].'-12-31"  readonly="readonly" />';
	echo '</td></tr>';
...
$comando="SELECT DISTINCT * FROM histf810 WHERE cue_codigo = '$cuenta' and substr(cue_codig2,1,2)='".substr($_POST['elano'],2,2)."'";
// echo $comando;
...
//		echo "<h2>Cuenta: ".$cuenta." ".$fila[cue_nombre].$cuento." <a href='javascript:print()'>  Imprimir</a>"."</h2>";
	...
	echo "<a target=\"_blank\" href=\"mayor_hispdf.php?cuenta=$cuenta&fechai=$fechai&fechaf=$fechaf&encero=$encero&\" onClick=\"info.html\', \'\',\'width=250, height=190\')\">Imprimir </a>"."</h2>"; 
		break;
	}
...
//	$lff=convertir_fecha($fechaf);
	$lfi=($fechai);
	$lff=($fechaf);
	$sql="SELECT * FROM histf820 WHERE com_cuenta = '$cuenta' AND ((com_fecha >= '$lfi') AND (com_fecha <= '$lff')) ORDER by com_fecha, com_refere";
	}
...
while ($fila = mysql_fetch_array($result)) :
	...
	echo "<td><a href='editasihis.php?asiento=".$fila["com_nrocom"]."'>". $fila["com_nrocom"]."</a><td>".$fila["com_descri"]."</td>";	
