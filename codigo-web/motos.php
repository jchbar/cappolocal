...
if ($accion == "Renovar") {	// seleccionar el tipo de prestamo nuevo de renovacion
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='motos.php?accion=Solicitar' name='form1' id='form1' method='post' onsubmit='return valpre(form1)'";
...
// 		bucle de listado
		...
		echo "<td class='centro'><a href='motos.php?accion=Ver&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/page_user_dark.gif' width='16' height='16' border='0' title='Consultar' alt='Consultar'/></a></td>";
		echo "<td class='centro'>";
		if ($row['renovacion']>1)
			if ($row['ultcan_sdp'] >= $row['renovacion']) {
				echo "<a href='motos.php?accion=Renovar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
				...
				echo "<a href='motos.php?accion=ReAjustar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
...
if (!$accion) {
	echo "<form action='motos.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
...
if (($accion == "Editar") or ($accion=="Renovar")) {	// muestra datos para prestamo
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='motos.php?accion=EscogePrestamo' name='form1' method='post' onsubmit='return valsoc(form1)'>";
	pantalla_prestamo($result,$cedula);
	echo "<input type = 'hidden' value ='".$cedula."' name='cedula'>";
	$elstatus=$_SESSION['elstatus'];
	echo '<fieldset><legend>Información Para Prestamo </legend>';
	$sqlprestamos="select * from sgcaf360 where cod_pres='062'";
/*
...
if ($accion == "ReAjustar") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='motos.php?accion=Reajuste' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
...
// phpinfo();
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='motos.php?accion=Concretar' name='form1' id='form1' method='post' onsubmit='return valpreres(form1)'";
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="motos.php?accion=Buscar">';
...
function solicitar_fiadores($elnumero,$lacedula)
{
	...
	echo "<form enctype='multipart/form-data' action='motos.php?accion=Fiadores' name='form1' id='form1' method='POST' onsubmit='return valfiadores(form1)'>";
...
//			echo '<td class="centro azul"><input type="checkbox" id="eliminar'.$registros.'" name="eliminar'.$registros.'" value='.$r_320["registro"] .' onClick="eliminafianza()"> </td>' ;
		...
		echo "<a href='motos.php?accion=Eliminar&cedula=".$lacedula."&nropre=".$elnumero."&registro=".$r_320['registro']."& '  onClick='return conf_elim_fiadores()'>";
...
*/
	...
	$sql_zapa="select * from sgcazapa where categoria='Motos' order by modelo";
...
//			echo '<td class="centro azul"><input type="textbox" id="cnt'.$registros.'" name="cnt'.$registros.'" value="0'.$registros .'"'; 
			echo '<td class="centro azul"><input type="textbox" id="cnt'.$registros.'" name="cnt'.$registros.'" value="1" readonly="readonly"'; 
			$registros++;
...
*/
	...
	echo '<input type="button" name="calculo" value="Calcular Cuota" onClick="ajax_call_motos()">	';
