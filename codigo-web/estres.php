...
echo "<form enctype='multipart/form-data' name='form1' action='estres.php' method='post'>";

echo "Estado de Resultados al mes de: ";
...
//		$encero=$_POST['encero'];
		...
		" as haber from sgcaf810 where (cue_nivel<='$nivelseleccionado') and (left(cue_codigo,1)>'3') order by cue_codigo, cue_nivel";
// 		echo $codigo."<br>";
		$result= mysql_query($codigo) or die('Error 810-5<br>'.mysql_error());
		$_SESSION['comando']=$codigo; // ['con_nivel'];
	echo '<h1>Estado de Resultados a '.	nombremes($elmes).'/'.$anoseleccionado.'.</h1> Nivel Detalle '.$nivelseleccionado;
...
//	echo "<a href='javascript:print()'>  Imprimir (Igual que en pantalla)  </a>";
	echo "<a target=\"_blank\" href=\"estrespdf.php?elmes=$elmes&nivelseleccionado=$nivelseleccionado&anoseleccionado=$anoseleccionado&$elejercicio=elejercicio&encero=$encero&messeleccionado=$messeleccionado&\" onClick=\"info.html\', \'\',\'width=250, height=190\')\">Imprimir PDF</a><br><br>"; 
