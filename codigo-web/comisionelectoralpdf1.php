...
*/
/*
select (@a:=@a+1) as linea, decanato as escuela, cod_prof, ced_prof, nombre, f_ing_capu as ingreso from (
	SELECT @a :=0, nombre as decanato, cod_prof, ced_prof, concat( trim( ape_prof ) , ' ', nombr_prof ) AS nombre, f_ing_capu FROM sgcaf200, escuelas where (
		(UPPER(statu_prof)="ACTIVO" or 
			UPPER(statu_prof)="JUBILA") and 
		(tipo_socio = "P" or tipo_socio = "E") 
		and escuelas.codigo = escuela
		and f_ing_capu <= '2019-03-31'
	)
	 order by nombre )Tabla1
*/
...
$sql="select (@a:=@a+1) as linea, escuela, cod_prof, ced_prof, nombre from (";
$sql.="SELECT @a :=0, ubicacion as escuela, cod_prof, cedula as ced_prof, nombre FROM listado, sgcaf200 where (cedula = ced_prof) and (";
	$sql.='UPPER(ubicacion)!="" ';
// $sql.='(tipo_socio = "P" or tipo_socio = "E") ';
$sql.=') order by cedula )Tabla1'; // . " limit 30"; //  limit 20";
...
$col_listado=0;
$header[1]='Linea Gral';
$header[0]='Linea Interna';
$header[2]='Codigo';
$header[3]='Cedula';
$header[4]='Apellidos y Nombres';
/*
$header[4]='Codigo';
...
$alto=5;
$salto=$alto;
$w=array(30,25,30,35,80); // ,15,50,15,15); 
$p[0]=10;
...
$np=$rsocio['escuela'];
$sqlesc="select ubicacion from listado group by ubicacion order by ubicacion";
$aesc=mysql_query($sqlesc);
while ($rescuela = mysql_fetch_assoc($aesc)){
	$np=$rescuela['ubicacion'];
	$rp = 0;
	$cont=0;
	$linterna = 0;
	...
	while ($rsocio = mysql_fetch_assoc($asocio)){
		if ($np==$rsocio['escuela']) {
			...
			$cont++;
			$pdf->SetY($linea);
			$pdf->SetX($p[1]);		$pdf->Cell($w[0],$alto,ceroizq(trim($rsocio['linea']),5),0,0,'C',0);
			$pdf->SetX($p[0]);		$pdf->Cell($w[0],$alto,ceroizq($linterna,5),0,0,'C',0);
			$pdf->SetX($p[2]);		$pdf->Cell($w[1],$alto,$rsocio["cod_prof"],0,0,'C',0); 
			$pdf->SetX($p[3]);		$pdf->Cell($w[2],$alto,$rsocio["ced_prof"],0,0,'C',0);  
			$pdf->SetX($p[4]);		$pdf->Cell($w[3],$alto,$rsocio["nombre"],0,0,'L');
		}
...
$hoy='31/12/2018';
$pdf->MultiCell(0,0,"Listado de Socios por Decanato al ".$hoy,0,C,0);
$pdf->SetY($linea);
$linea+=$alto;
$pdf->SetY($linea);
...
$pdf->Cell(200,$alto,'Decanato: '.$np,0,0,'L',0);
//  $linea+=$alto;
$pdf->SetX(220);
