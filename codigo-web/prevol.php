...
if ($accion == "Renovar") {	// seleccionar el tipo de prestamo nuevo de renovacion
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=Solicitar' name='form1' id='form1' method='post' onsubmit='return valpre(form1)'";
...
if ($accion == 'Buscar')  {
	...
	if ($lacedula) { //  != ' ') {
		...
		// print_r($row);
		/*
		if ($row['polizaactiva'] == 1)
			echo '<h3>Poliza de Vida: Activa</h3>';
		else 
			echo '<h1>Poliza de Vida: NO ACTIVA</h1>';
		$tsemanal=0;
		*/
		$accion = 'Editar'; 
	...
	// if (($_SERVER['REMOTE_ADDR']!='192.168.100.62') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.96') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.3') AND ($_ENV['COMPUTERNAME']!='JCHB-DM1'))	// icono para cancelar prestamo
		if (($_SERVER['REMOTE_ADDR']!='192.168.100.7') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.96') AND ($_SERVER['REMOTE_ADDR']!='192.168.1.3') AND ($_ENV['COMPUTERNAME']!='JCHB-DM1'))	// icono para cancelar prestamo
...
// 		bucle de listado
		while($row=mysql_fetch_assoc($rs)) {
			...
			echo "<td class='centro'><a href='prevol.php?accion=Ver&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/page_user_dark.gif' width='16' height='16' border='0' title='Consultar' alt='Consultar'/></a></td>";
			echo "<td class='centro'><a href='prevol.php?accion=ModificarCuota&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/modiftbl.jpg' width='16' height='16' border='0' title='Modificar Cuota' alt='Modificar Cuota'/></a></td>";
			echo "<td class='centro'>";
			if ($row['renovacion']>1)
				if ($row['ultcan_sdp'] >= $row['renovacion']) {
					echo "<a href='prevol.php?accion=Renovar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
					...
					echo "<a href='prevol.php?accion=ReAjustar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
				...
				echo "</td>";
				echo "<td class='centro'>";
				// if (($_SERVER['REMOTE_ADDR']=='192.168.100.62') OR ($_SERVER['REMOTE_ADDR']=='192.168.1.96') OR ($_SERVER['REMOTE_ADDR']=='192.168.100.62') or ($_ENV['COMPUTERNAME']=='JCHB-DM1'))	// icono para cancelar prestamo
				if (($_SERVER['REMOTE_ADDR']=='192.168.100.7') OR ($_SERVER['REMOTE_ADDR']=='192.168.1.96') OR ($_SERVER['REMOTE_ADDR']=='192.168.100.62') or ($_ENV['COMPUTERNAME']=='JCHB-DM1'))	// icono para cancelar prestamo
				//	jc / mendez viejo / mendez nuevo 
				{
					echo "<a href='prevol.php?accion=CancelarPrestamo&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
					echo "<img src='imagenes/informe.png' width='16' height='16' border='0' title='Cancelar Prestamo'  alt='Renovar' />";
					echo "</a>";
				}
				// if (($_SERVER['REMOTE_ADDR']=='192.168.1.9') OR ($_SERVER['REMOTE_ADDR']=='192.168.100.62') or ($_ENV['COMPUTERNAME']=='JCHB-DM1')) // icono para modificar neto a depositar prestamo
				// esta es la linea 
				if (($_SERVER['REMOTE_ADDR']=='192.168.100.7') OR ($_SERVER['REMOTE_ADDR']=='192.168.100.64') or ($_ENV['COMPUTERNAME']=='JCHB-DM1')) // icono para modificar neto a depositar prestamo
				{
	//				echo "<td class='centro'>";
					echo "<a href='prevol.php?accion=NetoDepositar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
...
//			echo "<td>".convertir_fechadmy($row['f_soli_sdp'])."</td>";
		...
		echo '<tr><td align="right" colspan="8">Total Descuento Quincenal: </td>';
...
if (!$accion) {
	echo "<form action='prevol.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
	// echo '<input name="cedula" type="text" id="cedula" value="V-05436619"  size="10" maxlength="10" />';
	echo '<input name="cedula" type="text" id="cedula" value="V-10846502"  size="10" maxlength="10" />';
...
if (($accion == "Editar") or ($accion=="Renovar")) {	// muestra datos para prestamo
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=EscogePrestamo' name='form1' method='post' onsubmit='return valsoc(form1)'>";
	...
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$sqls="select *, date_format(suspendido,'%d/%m/%y') as fs from suspende where ((cedula = '$micedula') and  (activo = 1) and (now() < suspendido))";
//	echo $sqls;
	$resuls=mysql_query($sqls);
	$vacio=(mysql_num_rows($resuls) > 0?true:false);
	while ($fila2 = mysql_fetch_assoc($resuls)) {
		echo '<h2>No se pudo descontar prestamo '.$fila2['prestamo']. ' enviado para '.$fila2['fallo'].' por un monto de '.number_format($fila2['monto'],2,'.',',').' suspendido hasta '.$fila2['fs']. ' reportado por '.$fila2['reporto'].'</h2>';
	}
	// print_r($rsocio);
	...
	$sqlprestamos="";
	$ininterrumpidos = $saldo_voluntario = 0;
	determinar_ahorro_voluntario($micedula, $rsocio['cod_prof'], $ininterrumpidos, $saldo_voluntario);
	echo 'Saldo Ahorro Voluntario : <strong>'.number_format($saldo_voluntario,2,'.',','). '</strong> Cobros ininterrumpidos <strong>'.number_format($ininterrumpidos,0,'.',',').'</strong><br><br>';
...
//		$sqlprestamos="select * from sgcaf360 where (retab_pres = 0) or (cod_pres='004') or (cod_pres='021') or (cod_pres='011') or (cod_pres='012') or (cod_pres='013') and ";
	...
	//////////////////
	if ($rsocio['polizaactiva'] == 0)
	{
		$sqlprestamos .= ") and (disponible_vida = 0";
	}
	else
	{ 
	}
	...
	$sqlprestamos = "select * from sgcaf360 where tipo='VOLUNTARIO'";
...
//	echo $sqlprestamos;
	...
	$sqlespecial="select * from sgcaf310, sgcaf360 where (cedsoc_sdp='".$micedula."' and codpre_sdp=cod_pres and stapre_sdp='A' and ! renovado) and (dcto_sem=0) and (f_1cuo_sdp <= '$fechadescuento') order by f_1cuo_sdp";
//	echo $sqlespecial;
...
//**********************************
	...
	if ($elprestamo == '066')
//		$sql="update sgcaf310 set netcheque = 29999 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
		$sql="update sgcaf310 set netcheque = 68600 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
	if ($elprestamo == 'solpr')
		// $sql="update sgcaf310 set netcheque = 49500 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
//		$sql="update sgcaf310 set netcheque = 69300 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
		$sql="update sgcaf310 set netcheque = 285000 where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
	*/
	$sql="update sgcaf310 set netcheque = round(netcheque,2) where cedsoc_sdp = '$micedula' and nropre_sdp='$referencia'";
	$resultado=mysql_query($sql);
...
//**********************************
	...
	if ($r_360['genera_pl'] == 1) 
		if ($r_360['nom_planilla'] == '') {
			...
			echo "<a target=\"_blank\" href=\"prevolpdf.php?cedula=$cedula\" onClick=\"info.html\', \'\',\'width=250, height=190\')\">Imprimir Planilla de Préstamo </a>"; 	
		}
		else {
...
if ($accion == "ReAjustar") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=Reajuste' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
...
if ($accion == "ModificarCuota") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=ModificaCuota' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
// phpinfo();
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=Concretar' name='form1' id='form1' method='post' onsubmit='return valpreres(form1)'";
...
if ($accion == "CancelarPrestamo") {	//  para cancelar prestamos
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=CancelaPrestamo' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
if ($accion == "NetoDepositar") {	//  para modificar neto a depositar de los prestamos
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=NetoDeposito' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
	$elnumero = $_GET['nropre'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$deci=$_SESSION['deci'];
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="prevol.php?accion=Buscar">';
...
function solicitar_fiadores($elnumero,$lacedula)
{
	...
	echo "<form enctype='multipart/form-data' action='prevol.php?accion=Fiadores' name='form1' id='form1' method='POST' onsubmit='return valfiadores(form1)'>";
...
//			echo '<td class="centro azul"><input type="checkbox" id="eliminar'.$registros.'" name="eliminar'.$registros.'" value='.$r_320["registro"] .' onClick="eliminafianza()"> </td>' ;
		...
		echo "<a href='prevol.php?accion=Eliminar&cedula=".$lacedula."&nropre=".$elnumero."&registro=".$r_320['registro']."& '  onClick='return conf_elim_fiadores()'>";
...
//	echo $registros. ' '.$r_310['fiadores'];
	...
	if ($r_310['genera_pl'] == 1) {
//		echo 'Preparando para la impresion<br>';
		echo "<a target=\"_blank\" href=\"prevolpdf.php?cedula=$lacedula\" onClick=\"info.html\', \'\',\'width=250, height=190\')\">Imprimir Planilla de Préstamo </a>"; 
	}
}
...
function determinar_ahorro_voluntario($micedula, $codigo, &$ininterrumpidos, &$saldo_voluntario)
{
	$sql_200="select hab_f_extr from sgcaf200 where cod_prof='$codigo'";
	$a_200=mysql_query($sql_200);
	$r_200=mysql_fetch_assoc($a_200);
	$saldo_voluntario = $r_200['hab_f_extr'];

	$sql = "SELECT * FROM `fhis200` WHERE cod_prof = '$codigo' AND substr(descri,1,4) = 'Vol.' ORDER by fecha";

// INSERT INTO `sgcaf360` VALUES ('VO1', 'VOLUNTARIO 1 MES', 2, 12.00, 0, 0, '', '', '', 1, 0, 3, 1, 1, 0, 0, 0, 1000.00, 0, 0, 1000.00, 'DIRECTO', '', '', 1, 1, 1, 0, 0, 0, 'VOL1', 'COMERCIAL', 1, '', 0, 1000.00, '', 0.00, '', 1, 0);

	$f_200=mysql_query($sql);
	// $rf_200=mysql_fetch_assoc($a_200);
	while($row=mysql_fetch_assoc($f_200)) {
		$ininterrumpidos ++;
		$rechazo = strpos($descr, 'FONDO');
		// var_dump($row);
		if ($rechazo > 0)
			$ininterrumpidos = 0;
	}
