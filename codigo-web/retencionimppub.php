...
$pdf->MultiCell(0,0,"Concepto Retención UCLA para Caja de Ahorro del ".convertir_fechadmy($fechaaporte),0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',8);
$pdf->SetFillColor(255,255,255);
$pdf->SetTextColor(0);
$linea+=5;
$header=array('Cédula','Retención','Cédula','Retención','Cédula','Retención','Cédula','Retención','Cédula','Retención');
// $w=array(15,15,15,15,15,15,15,15);
// $p=array(30,45,70,85,110,125,150,165);
$w=array(15,15,15,15,15,15,15,15,15,15);
$p=array(15,30,50,65,85,100,120,135,155,170);
$pdf->SetY($linea);
$pdf->SetFont('Arial','B',8);
$pdf->SetX(30);
...
$sql="select * from sgcaretuclai where (fecha ='$fechaaporte') order by cedula";
...
while ($row = mysql_fetch_array($result))
{
 ...
 //imprimo nombre, apellidos y localidad
	...
	$pdf->Cell($w[0+$columna],5,$row['cedula'],0,0,'C',0); 
	// $cont++; 
	// $acotado = trim($row["ape_prof"]).' '.trim($row["nombr_prof"]);
    // $pdf->Cell($w[2],5,$acotado,0,0,'LRTB',0);  
    // $asterisco = (((trim($row['pertenece']) == 'AO') or (trim($row['pertenece']) == 'JO'))?'*':'');
    // echo $asterisco;
	$pdf->SetX($p[1+$columna]);
	$pdf->Cell($w[1+$columna],5,number_format($row['monto'],2,".",","),0,0,'R',0);
...
// $sql="select sum(hab_prof) as monto, pertenece, count(pertenece) as cuantos from t_his200 where (fecha ='$fechaaporte') group by pertenece";
// $result=mysql_query($sql);
// while ($row = mysql_fetch_array($result))
// {
// 	$linea+=4;
...
// }
$archivo='retenciones_ucla/'.$fechaaporte.'r.pdf';
// $ip = $_SERVER['HTTP_CLIENT_IP'];
// if (!$ip) {$ip = $_SERVER['REMOTE_ADDR'];}
// $sql = "insert into sgcaretucla (fecha, registros, monto, procesado, ip, archivo) values ('$fechaaporte', $cont, $emonto, now(), '$ip', '$archivo')";
// // echo $sql;
// $result=mysql_query($sql);
$pdf->Output();
$pdf->Output($archivo);
