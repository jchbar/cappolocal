...
$pdf->MultiCell(0,0,"Indomiciliados Concepto Ahorro Voluntario para Caja de Ahorro del ".convertir_fechadmy($fechanomina),0,C,0);
$pdf->SetY($linea);
$pdf->SetFont('Arial','',8);
$pdf->SetFillColor(255,255,255);
$pdf->SetTextColor(0);
$linea+=5;
$header=array('Cédula','Nombre','Ahorro V.','Com.Bco','Cédula','Nombre','Ahorro V.','Com.Bco');
$comisionbancaria = 5;
// $w=array(15,15,15,15,15,15,15,15);
// $p=array(30,45,70,85,110,125,150,165);
$w=array(15,50,15,15,15,50,15,15,15,15);
$p=array(10,30,80,95, 113,130,170, 185);
$pdf->SetY($linea);
$pdf->SetFont('Arial','B',8);
$pdf->SetX(30);
...
$sql="select cedula, substr(concat(trim(ape_prof), ' ',trim(nombr_prof)),1,25) as nombre, hab_voluntario, pertenece from t_his200, sgcaf200 where (fecha ='$fechanomina') and (cedula = ced_prof) and (hab_voluntario > 0) order by cedula";
...
while ($row = mysql_fetch_array($result))
{
 ...
 //imprimo nombre, apellidos y localidad
	...
	$pdf->Cell($w[0+$columna],5,$row['cedula'].$asterisco,0,0,'C',0); 
	$pdf->SetX($p[1+$columna]);
	$pdf->Cell($w[1+$columna],5,$row['nombre'],0,0,'L',0); 
	// $cont++; 
    // $pdf->Cell($w[2],5,$acotado,0,0,'LRTB',0);  
    // echo $asterisco;
	$pdf->SetX($p[2+$columna]);
	$pdf->Cell($w[2+$columna],5,number_format($row['hab_voluntario'],2,".",","),0,0,'R',0);
	$monto_comisionbco= $row['comision']; // round($row['hab_prof']*($comisionbancaria / 100),2);
	$pdf->SetX($p[3+$columna]);
	$pdf->Cell($w[3+$columna],5,number_format($monto_comisionbco,2,".",","),0,0,'R',0);
	$emonto += $row['hab_prof'];
	$emonto2 += $monto_comisionbco;
	// $pdf->Cell($w[5],5,$row['pertenece'],0,0,'R',0);
...
// $sql="select sum(hab_prof) as monto, pertenece, count(pertenece) as cuantos from t_his200 where (fecha ='$fechanomina') group by pertenece";
...
// 	$pdf->SetX($w[2]);
// 	$pdf->Cell($w[2],5,$row['cuantos']. '->' .$acotado,0,0,'R',0); 
// 	$cont++; 
//     // $pdf->Cell($w[2],5,$acotado,0,0,'LRTB',0);  
// 	// $pdf->SetX($w[4]);
// 	$pdf->Cell($w[4],5,number_format($row['monto'],2,".",","),0,0,'R',0);
// }
$archivo='retenciones_ucla/'.$fechanomina.'i.pdf';
// $ip = $_SERVER['HTTP_CLIENT_IP'];
// if (!$ip) {$ip = $_SERVER['REMOTE_ADDR'];}
// $sql = "insert into sgcaretucla (fecha, registros, monto, procesado, ip, archivo) values ('$fechanomina', $cont, $emonto, now(), '$ip', '$archivo')";
// // echo $sql;
// $result=mysql_query($sql);
// $pdf->Output();
$pdf->Output($archivo);
