...
if ($accion == "Renovar") {	// seleccionar el tipo de prestamo nuevo de renovacion
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='viajes.php?accion=Solicitar' name='form1' id='form1' method='post' onsubmit='return valpre(form1)'";
...
// 		bucle de listado
		...
		echo "<td class='centro'><a href='viajes.php?accion=Ver&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'><img src='imagenes/page_user_dark.gif' width='16' height='16' border='0' title='Consultar' alt='Consultar'/></a></td>";
		echo "<td class='centro'>";
		if ($row['renovacion']>1)
			if ($row['ultcan_sdp'] >= $row['renovacion']) {
				echo "<a href='viajes.php?accion=Renovar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
				...
				echo "<a href='viajes.php?accion=ReAjustar&cedula=".$cedula."&nropre=".$row['nropre_sdp']."'>";
...
if (!$accion) {
	echo "<form action='viajes.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
...
if (($accion == "Editar") or ($accion=="Renovar")) {	// muestra datos para prestamo
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='viajes.php?accion=EscogePrestamo' name='form1' method='post' onsubmit='return valsoc(form1)'>";
	pantalla_prestamo($result,$cedula);
	echo "<input type = 'hidden' value ='".$cedula."' name='cedula'>";
	$elstatus=$_SESSION['elstatus'];
	echo '<fieldset><legend>Información Para Prestamo </legend>';
	$sqlprestamos="select * from sgcaf360 where cod_pres='039'";
/*
...
//	die ('primer dectuentp: '.$_POST['primerdcto']);
	$monpre_sdp = ($_POST['totalprestamo'] / 1.12) / 2;
	$inicial = $_POST['inicial'];

	$sql_200="select cod_prof from sgcaf200 where ced_prof='$cedula'";
	...
	$cuota = $_POST['totalprestamo'] / 2 ;
	$interes_sd = $_POST['interes_sd'];
	$lascuotas = $_POST['cuotas'];
	$micedula=substr($cedula,0,4).'.'.substr($cedula,4,3).'.'.substr($cedula,7,4);
	$laparte=$r_200['cod_prof'];
	$laparte=substr($laparte,0,5);

	echo 'cedula'.$micedula. ' -' .$laparte; /////////////// 
...
/*
	$sql_310="select count(nropre_sdp) as cantidad, codsoc_sdp from sgcaf310 where (cedsoc_sdp='$micedula') group by cedsoc_sdp";
// 	echo $sql_310;
	$a_310=mysql_query($sql_310);
	$elnumero=mysql_fetch_assoc($a_310);
	$laparte=$elnumero['codsoc_sdp'];
...
//	echo $sql_acta;
	...
	/////////////////
	if ($_POST['tiposocio'] == 'Activo')
	{
		$primerdcto='2015-04-30';
		$inicial=$intereses_diferidos=0;

		echo "Creando préstamo nuevo numero <strong>$elnumero</strong><br>";
		$sql="insert into sgcaf310 (codsoc_sdp, cedsoc_sdp, nropre_sdp, codpre_sdp, f_soli_sdp, f_1cuo_sdp, monpre_sdp, monpag_sdp, nrofia_sdp, stapre_sdp, tipo_fianz, cuota, nrocuotas, interes_sd, cuota_ucla, netcheque, nro_acta, fecha_acta, ip, inicial, intereses) values ('$laparte', '$micedula', '$elnumero','$elprestamo','$hoy', '$primerdcto', $monpre_sdp, 0, 0, '$estatus', '',$cuota, 1, $interes_sd, $cuota, $monpre_sdp, '$nroacta', '$fechaacta', '$ip', $inicial, $intereses_diferidos)";
//	echo $sql; // 1er prestamo
...
*/
		$elnumero=numero_prestamo($micedula, $laparte);
	// fin de generar nuevo numero
		$primerdcto='2014-08-30';
		$sql="insert into sgcaf310 (codsoc_sdp, cedsoc_sdp, nropre_sdp, codpre_sdp, f_soli_sdp, f_1cuo_sdp, monpre_sdp, monpag_sdp, nrofia_sdp, stapre_sdp, tipo_fianz, cuota, nrocuotas, interes_sd, cuota_ucla, netcheque, nro_acta, fecha_acta, ip, inicial, intereses) values ('$laparte', '$micedula', '$elnumero','$elprestamo','$hoy', '$primerdcto', $monpre_sdp/2, 0, 0, '$estatus', '',$cuota/2, 2, $interes_sd, $cuota/2, $monpre_sdp, '$nroacta', '$fechaacta', '$ip', $inicial, $intereses_diferidos)";
//	echo $sql; // 1er prestamo
		$resultado=mysql_query($sql);
		$_SESSION['observa']='Descuentos (1er y 3er) 25% Diciembre 2014 y Agosto 2015 Bs. '.number_format(($cuota/2),2,'.',',').'/ 2do = 50% Abril 2015 Bs. '.number_format(($cuota),2,'.',',');
	}
	else 
	{
		$primerdcto='2013-08-30';
		$inicial=$intereses_diferidos=0;
		$sql="insert into sgcaf310 (codsoc_sdp, cedsoc_sdp, nropre_sdp, codpre_sdp, f_soli_sdp, f_1cuo_sdp, monpre_sdp, monpag_sdp, nrofia_sdp, stapre_sdp, tipo_fianz, cuota, nrocuotas, interes_sd, cuota_ucla, netcheque, nro_acta, fecha_acta, ip, inicial, intereses) values ('$laparte', '$micedula', '$elnumero','$elprestamo','$hoy', '$primerdcto', $monpre_sdp*2, 0, 0, '$estatus', '',$cuota, 2, $interes_sd, $cuota, $monpre_sdp, '$nroacta', '$fechaacta', '$ip', $inicial, $intereses_diferidos)";
//echo $sql; // 1er prestamo
		$resultado=mysql_query($sql);
		$_SESSION['observa']='Descuentos (1er y 2do) Diciembre 2014 y Agosto 2015 Bs. '.number_format(($cuota/2),2,'.',',');
	}

	$_SESSION['viaje']=$_POST['date3'];
	$viaje=$_POST['date3'];
	
	$comando="insert into sgcaviaj (cedtitular, cedbene, nombre, fecnac, fecviaje, edad) values ('$micedula', '$micedula', '".trim($r_200['ape_prof']). ' '.$r_200['nombr_prof']."', '".$r_200['nacimiento']."', '$viaje', '99')";
//			echo $comando.'<br>'; 
	$resultado=mysql_query($comando);
	for ($i=0;$i<10;$i++)
	{
		$cedben=$_POST['cedulas'.$i];
		$nombre=$_POST['nombre'.$i];
		$nace=$_POST['nacimiento'.$i];
		$viaje=$_POST['date3'];
		$edad=$_POST['edad'.$i];
		
		if (strlen(trim($nombre)) > 3)
		{
			$comando="insert into sgcaviaj (cedtitular, cedbene, nombre, fecnac, fecviaje, edad) values ('$micedula', '$cedben', '$nombre', '$nace', '$viaje', '$edad')";
//			echo $comando.'<br>'; 
			$resultado=mysql_query($comando);
		}
	}
...
//////////////////
		...
		$monpre_sdp = ($_POST['totalprestamo'] / 1.12) ;
		$debe = $monpre_sdp;
		if ($r_360['int_dif'] == 1) {
...
if ($accion == "ReAjustar") {	//  para aquellos que solo aumentan el monto y varian la cuota
	$mostrarregresar=1;
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='viajes.php?accion=Reajuste' name='form1' id='form1' method='post' onsubmit='return valajuste(form1)'";
	$cedula = $_GET['cedula'];
...
// phpinfo();
	echo '<div id="div1">';
	echo "<form enctype='multipart/form-data' action='viajes.php?accion=Concretar' name='form1' id='form1' method='post' onsubmit='return valpreres(form1)'";
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="viajes.php?accion=Buscar">';
...
function solicitar_fiadores($elnumero,$lacedula)
{
	...
	echo "<form enctype='multipart/form-data' action='viajes.php?accion=Fiadores' name='form1' id='form1' method='POST' onsubmit='return valfiadores(form1)'>";
...
//			echo '<td class="centro azul"><input type="checkbox" id="eliminar'.$registros.'" name="eliminar'.$registros.'" value='.$r_320["registro"] .' onClick="eliminafianza()"> </td>' ;
		...
		echo "<a href='viajes.php?accion=Eliminar&cedula=".$lacedula."&nropre=".$elnumero."&registro=".$r_320['registro']."& '  onClick='return conf_elim_fiadores()'>";
...
setActiveStyleSheet(document.getElementById("defaultTheme"), "green");
	...
	Fecha Viaje:</b> <input type="text" name="date3" id="sel3" size="12" readonly
><input type="reset" value=" ... "
onclick="return showCalendar('sel3', '%Y-%m-%d');"><br />
	<?php 
	echo "<input type = 'hidden' value ='".$_POST['cedula']."' name='cedula'>";
	echo 'Numero de Noches pernocta del titular <td class="centro azul"><input type="textbox" id="nochest" name="noches" value="2" maxlength="2" readonly="readonly"></td>'; 
	echo 'Tipo de Socio <select id="tiposocio" name="tiposocio" size="1">';
	echo '<option value="Activo" selected> Activo</option>';
	echo '<option value="Jubilado" >Jubilado</option>';
  	echo '</select> ';	

	echo '<table>';
	echo '<tr><td>Cedula</td><td>Apellidos / Nombres</td><td>Fecha</td><td>#Noches</td><td>Edad</td><td>Base Pago</td></tr>';
	for ($i=0;$i<10;$i++)
	{
		$registros=$i;
		echo '<tr>';
		echo '<td class="centro azul"><input type="textbox" id="cedulas'.$registros.'" name="cedulas'.$registros.'" value=""></td>'; 
		echo '<td class="centro azul"><input type="textbox" id="nombre'.$registros.'" name="nombre'.$registros.'" value="" maxlength="30"></td>'; 
		echo '<td class="centro azul"><input type="textbox" id="nacimiento'.$registros.'" name="nacimiento'.$registros.'" value="" maxlength="10"></td>'; 
		echo '<td class="centro azul"><input type="textbox" id="noches'.$registros.'" name="noches'.$registros.'" value="2" maxlength="2" readonly="readonly"></td>'; 
		echo '<td class="centro azul"><input type="textbox" id="edad'.$registros.'" name="edad'.$registros.'" value="0" maxlength="2" readonly="readonly"></td>'; 
		echo '<td class="centro azul"><input type="textbox" id="pago'.$registros.'" name="pago'.$registros.'" value="0" maxlength="2" readonly="readonly"></td>'; 
		echo '</tr>';
	}
	echo '</table>';
	echo "<input type = 'hidden' value ='10' name='registros' id='registros'>";
	echo "Total Prestamo Viaje: <input type = 'textbox' value ='10' name='totalprestamo' id='totalprestamo'>";
	echo '<td rowspan="5"><textarea name="observacion" type="text" id="observacion" cols="35" rows="04" maxlength="95" value=""/> </textarea></td>';
	...
	echo '<input type="button" name="calculo" value="Calcular Cuota" onClick="ajax_call_viajes()">	';
...
//	echo '<div id="contenido2"></div>';
	...
	echo '<tr><td>Totales</td><td align="right" >'.$ta.'</td><td align="right" >'.$tm.'</td><td align="right" >'.$ti.'</td></tr>';
	echo '<tr><td>Total General</td><td align="right" colspan="3">'.($ta+$tm).'('.(200-($ta+$tm)).')</td></tr>';
	echo '</table>';
	echo '</div>';
