...
if ((! $_POST['procesar']) and (! $_FILES['archivo']['name'])) {
	echo '<fieldset><legend>Informacion para procesar archivo para Medical Assist</legend>';
	echo '<form action="medical.php" method="post" name="form1" enctype="multipart/form-data">';
	echo 'Fecha de Descuento Prestamo Medical Assist ';
?>
...
//	echo '<input type="checkbox" name="nominasemanal" value = "1" checked/> Nomina Semanal<br />';
	echo 'Indique el archivo (CSV) que contiene la informacion de Medical Assist <input name="archivo" type="file" value="Examinar"><br>';
...
if(@$_FILES['archivo']['name']!=='') // {
	$salida='hospitalito/sica_'.$_FILES['archivo']['name'];
	$archivosalida=fopen ($salida, "w+");
	$nueva_ruta='hospitalito/';
	$ruta_total = $_SERVER['DOCUMENT_ROOT'].$nueva_ruta;
	$ruta_total = $_SERVER['DOCUMENT_ROOT']."/cajalocal/hospitalito/".$_FILES['archivo']['name'];
//	echo $ruta_total;
...
/*
	$salida='hospitalito/sica_'.$_POST['archivosalida'];
...
$lines = file('hospitalito/'.$archivo_name);
$faltoalguno=0;
...
$cuento='Cargo en Cuenta Prestamos Medical de fecha '.$b;
...
$sql="select * from sgcaf360 where cod_pres='065'";
...
$interes=$cuentas['i_max_pres'];

$sql="select * from sgcaf000 where tipo='AbonoHospital'";
$result=mysql_query($sql); // or die ("<p />El usuario $usuario no pudo conseguir la cuenta x pagar<br>".mysql_error()."<br>".$sql);
$cuentas=mysql_fetch_assoc($result);
$cuentahospital=trim($cuentas['nombre']); // .'-'.substr($laparte,1,4);
...
//		$totalesgral+=($datos[3]*100)/100;
		...
		$monto = $cuota;
		$totalesgral += $monto;
		$cuota = ($cuota * (1 + ($interes / 100)));
		$nrocuota = 2;
		$cuota = (($monto* (1 + ($interes / 100))) / $nrocuota);
		// fin de generar nuevo numero
		$hoy = date("Y-m-d");
		$sql_310="INSERT INTO sgcaf310 (codsoc_sdp, cedsoc_sdp, nropre_sdp, codpre_sdp, f_soli_sdp, f_1cuo_sdp, ultcan_sdp, monpre_sdp, stapre_sdp, cuota, nrocuotas, cuota_ucla, ip) VALUES ('".$r200['cod_prof']."', '".$micedula."', '$elnumero', '065', '$hoy', '$b', 0, $monto, 'A', $cuota, '$nrocuota', $cuota, '$ip')";
//		echo $sql_310.'<br>';
		$saldo=$monto;
...
//			echo $sql810;
	...
	$debe=$totalesgral;
	$cuenta1=$cuentahospital;
	agregar_f820($asiento, $b, '-', $cuenta1, "Relacion Medical de Fecha ".$b, $debe, $haber, 0,$ip,0,$referencia,'','S',0); 
}
