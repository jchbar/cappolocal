...
//	$sql="select com_cuenta, com_debcre, sum(com_monto1) as debe, sum(com_monto2) as haber from sgcaf820 where month(com_fecha)=$elmes group by com_cuenta order by com_cuenta";
	...
	$sql="select left(com_cuenta,$numero) as reducido, com_cuenta, com_debcre, sum(com_monto1) as debe, sum(com_monto2) as haber from sgcaf820 where month(com_fecha)=$elmes group by reducido order by reducido";
// 	echo $sql;
...
// echo "comando ejecutado en $tiempo segundos";
	...
	if (mysql_num_rows($rs) > 0) {
		$elsql="UPDATE sgcaf830 SET enc_debe='$fila[debe]', enc_haber='$fila[haber]', enc_item='$fila[items]'";
		if ($agregar != 'E') 
			$elsql.=", enc_fecha='$pcom_fecha' ";
		$elsql.=" WHERE enc_clave = '$pcom_nrocom'";
// 		die($elsql);
...
{
	$fianzas=0;
	$sql="select (monto_fia-monlib_fia) as saldo from sgcaf320 where codsoc_fia = '$elcodigo' and (tipmov_fia = 'F') group by codsoc_fia ";
	$resultado2=mysql_query($sql);
	if (mysql_num_rows($resultado2) > 0)
	{
		$fila2 = mysql_fetch_assoc($resultado2);
