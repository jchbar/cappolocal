...
?>
<script src="ajaxretuclacierre.js" type="text/javascript"></script>
...
if (!$accion) {
	...
	$viejo=$viejo['viejos'];
	$sql='SELECT fecha, count(fecha) as cuantos FROM `t_his200` WHERE hab_prof <> 0 and (pertenece <> "") group by fecha';
	$a_sql=mysql_query($sql);
	// echo $sql; 
	echo "<form action='aicn.php?accion=Cerrar' name='form1' method='post' enctype='multipart/form-data' onsubmit='return realizar_cierre(form1)'>";
	?>
	<fieldset><legend>Información Para Cierre de Retenciones </legend>
	<table>
	...
	echo '<fieldset><legend>Resumen Para Cierre Retenciones UCLA</legend>';
...
if ($accion=='Cerrar') 
{
	...
	echo 'Generando Reporte Indomiciliados<br>';
	include_once('retencionindomiciliados.php');
	echo 'Generado Reporte Indomiciliados<br>';
	...
	if ($ValorTotal > 0) 
	{
		...
		$desc = 'Cierre nominas p/socios indomiciliados';
		...
		while ($r_amor = mysql_fetch_assoc($a_amor)) 
		{
			...
			$monto_retencion= $r_amor['hab_prof'];
			$monto_comisionbco= round($r_amor['hab_prof']*($comisionbancaria / 100),2);
			...
			if ($monto_retencion > 0)
			{
				...
				$sql = "update t_his200 set hab_prof = 0 where numreg = '$registro'";
				...
				$sql = "insert into suspende (codigo, nombre, prestamo, monto, cedula, reporto, fallo, activo, ingresado, suspendido, motivo_in) values ('$codigo', '$nombre', 'DOMIC. RET. UCLA', $monto_retencion, '$lacedula', 'Sistema Banco','Indomiciliado Ret.UCLA',1,'$fechanomina', '$suspension', '')";
...
function registrar_paso($resultado, $registro, $lacomision)
{
	...
	if ($estado == 0) // paso
	{
		...
		$sql3="update sgcaf200 set hab_f_prof = hab_f_prof + ".$monto.", ultap_prof='".$registro['fecha']."', ultapm_prof='".$monto."' where cod_prof ='". $registro['cod_prof']."'";
