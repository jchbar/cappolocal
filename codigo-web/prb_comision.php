<?php
/*
INSERT INTO `sica`.`sgcaf810` (`cue_codigo`, `cue_nombre`, `cue_nivel`, `cue_saldo`, `cue_deb01`, `cue_cre01`, `cue_deb02`, `cue_cre02`, `cue_deb03`, `cue_cre03`, `cue_deb04`, `cue_cre04`, `cue_deb05`, `cue_cre05`, `cue_deb06`, `cue_cre06`, `cue_deb07`, `cue_cre07`, `cue_deb08`, `cue_cre08`, `cue_deb09`, `cue_cre09`, `cue_deb10`, `cue_cre10`, `cue_deb11`, `cue_cre11`, `cue_deb12`, `cue_cre12`, `cue_deb13`, `cue_cre13`, `cue_deb14`, `cue_cre14`, `cue_deb15`, `cue_cre15`, `cue_deb16`, `cue_cre16`, `cue_deb17`, `cue_cre17`, `cue_deb18`, `cue_cre18`, `cue_deb19`, `cue_cre19`, `cod_viejo`, `codsudeca`, `nomsudeca`) VALUES ('2-01-01-06-06-06', 'SISTEMAS', '6', '0', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '');
INSERT INTO `sica`.`sgcaf810` (`cue_codigo`, `cue_nombre`, `cue_nivel`, `cue_saldo`, `cue_deb01`, `cue_cre01`, `cue_deb02`, `cue_cre02`, `cue_deb03`, `cue_cre03`, `cue_deb04`, `cue_cre04`, `cue_deb05`, `cue_cre05`, `cue_deb06`, `cue_cre06`, `cue_deb07`, `cue_cre07`, `cue_deb08`, `cue_cre08`, `cue_deb09`, `cue_cre09`, `cue_deb10`, `cue_cre10`, `cue_deb11`, `cue_cre11`, `cue_deb12`, `cue_cre12`, `cue_deb13`, `cue_cre13`, `cue_deb14`, `cue_cre14`, `cue_deb15`, `cue_cre15`, `cue_deb16`, `cue_cre16`, `cue_deb17`, `cue_cre17`, `cue_deb18`, `cue_cre18`, `cue_deb19`, `cue_cre19`, `cod_viejo`, `codsudeca`, `nomsudeca`) VALUES ('2-01-01-06-06-06-0001', 'HEROS SERVICIOS', '7', '0', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '');
INSERT INTO `sica`.`sgcaf000` (`tipo`, `nombre`, `idregistro`) VALUES ('SisxPagar', '2-01-01-04-06-01-0021', NULL);

cedula 12369466 codigo 00261
*/
...
?>
<script src="ajaxretucla_comision.js" type="text/javascript"></script>
...
if (!$accion) {
	...
	$viejo=$viejo['viejos'];
	$sql='SELECT fecha, sum(comision) as monto, count(fecha) as cuantos, pertenece FROM t_his200 where  (fecha > "'.$viejo.'") and pertenece IS NOT NULL group by fecha desc ';
	$a_sql=mysql_query($sql);
	// echo $sql; 
	echo "<form action='prb_comision.php?accion=Abonar' name='form1' method='post' enctype='multipart/form-data' onsubmit='return realizar_abono(form1)'>";
	?>
	<fieldset><legend>Información Para Relacionar Comision Bancaria</legend>
	<table>
	...
	<tr><th width="50">Fecha</th><th width="80">Monto</th><th width="80">Cantidad</th><th width="40">Procesar</th>
	...
	while($r=mysql_fetch_assoc($a_sql)) {
		...
		// $fecha.= ',';
		// $fecha .= "'".$r['pertenece']."'";
		echo '<td class="centro azul"><h1><input type="checkbox" id="cancelar'.$registros.'" name="cancelar'.$registros.'" onClick="amor_cap_comision('.$fecha.\')"';
		echo '></td></tr>' ;
	}
	...
	echo '<fieldset><legend>Resumen Para Retenciones UCLA</legend>';
	...
	echo '</table>';
	echo '</legend>';
	echo '<input type="submit" name="Submit" value="Realizar Abono Comision Retenciones (Asientos Contables)" />';
	echo '</form>';
}	// if (!$accion) 
if ($accion=='Abonar') 
{
	...
	for ($veces=0;$veces<3;$veces++)	// recorrer los 3 archivos y procesar
	{
		...
		{
			$salida='devolucion_banco_ucla/sica_'.$_FILES['archivo']['name'][$veces];
			$archivosalida=fopen ($salida, "w+");
			$nueva_ruta='devolucion_banco_ucla/';
			$ruta_total = $_SERVER['DOCUMENT_ROOT'].$nueva_ruta;
			$ruta_total = $_SERVER['DOCUMENT_ROOT']."/cajaweb/devolucion_banco_ucla/".$_FILES['archivo']['name'][$veces];
	...
	// die('espero');
		...
		for ($i=0;$i<$registros;$i++)	// no es necesarios revisar el check si aparece es porq estan seleccionados para hacer el asiento 
		{
			...
			if (!empty($$variable)) 
			{
				...
				$sql="select * from t_his200 where (hab_prof > 0) and (fecha ='$fechanomina') order by numreg"; 
				// echo $sql.'<br>';
				$a_amor=mysql_query($sql);
				...
				if ($ValorTotal > 0) 
				{
					...
					crear_encabezado($elasiento1,$b,'Comision Retencion UCLA p/'.$concepto);
					$ofecha=$b;
					...
					while ($r_amor = mysql_fetch_assoc($a_amor)) 
					{
						...
						if ($estado != '2')
						{
							if ($calcular_comision)
								$lacomision = round($r_amor['hab_prof'] * ($comisionbancaria / 100),2);
							else
								$lacomision = 0;
							$mcomision += $lacomision;
							if ($estado == 0) 
							{
								$mpasaron += $r_amor['hab_prof'];
							}
							else 
							{
								$mnopasaron += $r_amor['hab_prof'];
							}
							registrar_paso($resultado, $r_amor, $lacomision);
						}
					}

					$desc='Nomina Cobrada Comision Retencion del '.$otrafecha;
					$referencia = $tipo;
					$asiento = $elasiento1;
					// parte 1
...
function servicio_sistema($b, $mcomision)
{
	$ofecha = explode("-", $b);
	$elasiento1=$ofecha[0].$ofecha[1].$ofecha[2].'456';
	$desc='Honorario Profesional Servicio Adaptacion Sistema '.$b;
	crear_encabezado($elasiento1,$b,$desc);
	$desc='Honorario Profesional Serv. Adapt. Sistema '.$b;
	hacer_asiento('ComisionDom', $mcomision, '+', $desc, $elasiento1, $b, $referencia);
	hacer_asiento('SisxPagar', $mcomision, '-', $desc, $elasiento1, $b, $referencia);
}
...
function registrar_paso($resultado, $registro, $lacomision)
{
	...
	$monto_retencion = $monto = $registro['hab_prof'];
	$monto -= $lacomision;
	$monto -= $lacomision;
	$dcto2 = $lacomision; 
	...
	$sql1="insert into fhis200 (cod_prof, hab_prof, fecha, descri, pago, ip, procesado, monto_retencion, dcto01, dcto02) values ('$codigo', $monto, '$fecha', '$descripcion', '$fechadelpago', '$ip', now(), $monto_retencion, $lacomision, $dcto2)";
	$sql2="update t_his200 set hab_prof = 0 where numreg =". $registro['numreg'];
	// restar comision bancaria ?
	$sql3="insert into fhis200 (cod_prof, hab_prof, fecha, descri, pago, ip, procesado) values ('$codigo', ($lacomision*-1), '$fecha', '$descripcion3', '$fechadelpago', '$ip', now())";
	...
	if ($estado == 0) // paso
	{
		...
		$sql3="update sgcaf200 set hab_f_prof = hab_f_prof + ".$monto.", ultap_prof='".$registro['fecha']."', ultapm_prof='".$monto."', domiciliacion_buena = domiciliacion_buena + 1,  domiciliacion_falla = 0 where cod_prof ='". $registro['cod_prof']."'";
	...
	else
	{
		...
		$sql1="insert into fhis200 (cod_prof, hab_prof, fecha, descri, pago, ip, procesado, monto_retencion, dcto01, dcto02) values ('$codigo', ($monto*-1), '$fecha', '$descripcion', '$fechadelpago', '$ip', now(), $monto_retencion, $lacomision, $dcto2)";
		// $r1=mysql_query($sql1) or die ("El usuario $usuario no tiene permiso para HF200-1.<br>".mysql_error()."<br>".$sql1);
		// contador historial retenciones
