...
$pdf->SetY($linea);
$pdf->SetX(0);
$sql="select codsoc_sdp, cedsoc_sdp,nom_prof, ape_prof from sgcaf310, sgcaf200 where ((tipo_socio='E' or tipo_socio='P') and (codsoc_sdp = cod_prof) and (UPPER(statu_prof)='ACTIVO' OR UPPER(statu_prof)='JUBILA')) and ((codpre_sdp='071' and f_1cuo_sdp='2017-12-08') and (stapre_sdp='A' or stapre_sdp='C')) order by cedsoc_sdp";
...
while ($fila = mysql_fetch_assoc($result)) {
	...
	$pdf->SetY($linea+2);
	$pdf->SetX($lado);
	$pdf->Cell(50,7,'AHORRO SALUD',0,2,'C',1);
	...
	$pdf->SetX($lado);
	$pdf->SetFont('Arial','',8);
	$pdf->Image('logo_gustavo.jpg',$lado+55,$linea+8,18,18,jpg);
//	$pdf->Cell(50,30,substr($fila['codsoc_sdp'],1,4),0,0,'C',0);
	$pdf->Cell(50,30,trim($fila['ape_prof']) . ' '.trim($fila['nom_prof']),0,0,'C',0);
